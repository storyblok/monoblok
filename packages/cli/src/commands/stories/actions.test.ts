...
describe("stories/actions", () => {
  ...
  describe("fetchStories", () => {
    ...
    it("should handle unauthorized errors", async () => {
      ...
      expect(handleAPIError).toHaveBeenCalledWith("pull_stories", expect.anything());
  ...
  describe("prefetchTargetStoriesByKeys", () => {
    ...
    const buildRemoteStory = (overrides: {
      id: number;
      uuid: string;
      full_slug: string;
      is_folder?: boolean;
    ...
    it("fetches stories by slug and indexes them by normalized full_slug and id", async () => {
      ...
      expect(result.bySlug.get("home")).toEqual([{ id: 7, uuid: "uuid-7", is_folder: false }]);
    ...
    it("fetches stories by id and indexes them by id and full_slug", async () => {
      ...
      expect(result.bySlug.get("pricing")).toEqual([{ id: 99, uuid: "uuid-99", is_folder: false }]);
    });
    ...
    it("deduplicates a story returned by both slug and id queries", async () => {
      ...
      server.use(
        http.get("https://mapi.storyblok.com/v1/spaces/:spaceId/stories", () =>
          HttpResponse.json({ stories: [remote] }, { headers: { Total: "1", "Per-Page": "100" } }),
        ),
      );
