 ...
 * `'My Layout/Heros'`), walking each group's `parent_uuid` chain. Unlike
 * {@link buildGroupPathByUuid}, segments keep their original names: this map
 * feeds the `folder` literals in generated *types*, where `Block['folder']` is
 * documented as a display-name path. Both sides of a `MatchesFolder` comparison
 * (a block's `folder` and a field's `allow: [{ folder }]`) come from this same
 * map, so they always narrow consistently.
 *
 * Cycle handling mirrors {@link buildGroupPathByUuid}: a self-referential or
 * cyclic `parent_uuid` chain is cut and the group is treated as a path root.
 */
export function buildGroupDisplayPathByUuid(folders: ComponentFolder[]): Map<string, string> {
  const byUuid = new Map(folders.map(folder => [folder.uuid, folder]));
  const segmentsByUuid = new Map<string, string[]>();

  function pathFor(uuid: string | null, visited: Set<string>): string[] {
    if (!uuid) { return []; }
    const cached = segmentsByUuid.get(uuid);
    if (cached) { return cached; }
    const folder = byUuid.get(uuid);
    if (!folder) { return []; }
    if (visited.has(uuid)) { return []; }
    visited.add(uuid);
    const path = [...pathFor(folder.parent_uuid, visited), folder.name];
    segmentsByUuid.set(uuid, path);
    return path;
  }
