...
describe("logs prune command", () => {
  ...
  it("should delete all logs when keep is 0 (default)", async () => {
    ...
    expect(console.info).toHaveBeenCalledWith(expect.stringContaining("Deleted 3 log files"));
  ...
  it("should keep specified number of recent logs", async () => {
    ...
    expect(console.info).toHaveBeenCalledWith(expect.stringContaining("Deleted 1 log file"));
    const remainingFiles = Object.keys(vol.toJSON()).filter((path) => path.includes(".jsonl"));
  ...
  it("should not delete logs when keep count equals total", async () => {
    ...
    expect(console.info).toHaveBeenCalledWith(expect.stringContaining("Deleted 0 log files"));
    const remainingFiles = Object.keys(vol.toJSON()).filter((path) => path.includes(".jsonl"));
