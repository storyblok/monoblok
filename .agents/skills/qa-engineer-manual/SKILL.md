...
- **Trust the seed's exit status, not just its log.** The seed now verifies each staged resource
  ...
  reports success but creates nothing, so check the exit status. Piping the run into `tee` or `grep`
  hides it behind the last command's status: run `set -o pipefail` first, the one form that reads
  the same in bash and zsh (`$PIPESTATUS` is bash-only, `$pipestatus` zsh-only).
...
| `./scripts/save-storyblok-session.mjs` | Opens a headed browser so you can log in to app.storyblok.com once; saves the session to `.storyblok-qa/session.json` for Playwright harnesses and one-off scripts to reuse. Run with `node`, no env needed; set `STORYBLOK_QA_CDP_URL` to reuse a Chrome already running with remote debugging when there is no display. Never reads or stores a credential. |
| `./scripts/configure-space.sh`         | Reads a space's Visual Editor preview domain, and sets it with `--domain <https-url> --confirm`. Read-only without `--confirm`; refuses non-https domains. Accepts `--space <id>`.                                                                                                                                                                            |
