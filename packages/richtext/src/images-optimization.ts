...
export function optimizeImage(
  src: string,
  options?: boolean | Partial<StoryblokRichTextImageOptimizationOptions>,
): { src: string; attrs: Record<string, any> } {
  if (!options) {
    return { src, attrs: {} };
  }
  let w = 0;
  ...
  function validateAndPushFilterParam(
    value: number,
    min: number,
    max: number,
    filter: string,
    ...
    if (typeof value !== "number" || value <= min || value >= max) {
      console.warn(
        `[StoryblokRichText] - ${filter.charAt(0).toUpperCase() + filter.slice(1)} value must be a number between ${min} and ${max} (inclusive)`,
      );
    } else {
