  ...
  describe("readComponentsFiles", () => {
      ...
      it("should throw FileSystemError when directory does not exist", async () => {
        ...
        try {
          ...
          expect((error as FileSystemError).message).toContain(
            "No local components found for space source-space",
          );
          expect((error as FileSystemError).message).toContain(
            "storyblok components pull --space source-space",
          );
          expect((error as FileSystemError).message).toContain(
            "storyblok components push --space <target_space> --from source-space",
          );
        }
      });
    });
    ...
    describe("separate files mode", () => {
      it("should read components from separate files without suffix", async () => {
        ...
        vol.fromJSON({
          ...
          "/mock/path/components/source-space/groups.json": JSON.stringify([
            mockGroup1,
            mockGroup2,
          ]),
    ...
    describe("consolidated files mode", () => {
      ...
      it("should read components from consolidated file with suffix", async () => {
        ...
        vol.fromJSON({
          "/mock/path/components/source-space/components.dev.json": JSON.stringify([
            mockComponent1,
            mockComponent2,
          ]),
          "/mock/path/components/source-space/groups.dev.json": JSON.stringify([mockGroup1]),
      ...
      it("should read consolidated file with custom filename", async () => {
        ...
        vol.fromJSON({
          "/mock/path/components/source-space/my-schemas.json": JSON.stringify([
            mockComponent1,
            mockComponent2,
          ]),
        });
    ...
    describe("content-based classification", () => {
      ...
      it("should skip unrecognized items without error", async () => {
        const dataWithUnknown = [mockComponent1, { foo: "bar", baz: 42 }, { unknown_field: true }];
    ...
    describe("path resolution", () => {
      it("should use correct path resolution for source space", async () => {
        ...
        vol.fromJSON({
          "/custom/base/path/components/my-source-space/components.json": JSON.stringify([
            mockComponent1,
          ]),
        });
      ...
      it("should read from different source space than target space", async () => {
        ...
        vol.fromJSON({
          "/mock/path/components/production-space/components.json": JSON.stringify([
            mockComponent1,
            mockComponent2,
          ]),
