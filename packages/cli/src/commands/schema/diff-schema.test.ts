...
/**
 * Builds a {@link NormalizedSchema} from entity arrays. `translations` carries the
 * uuid/id → name maps a remote space contributes; a schema loaded from code has none.
 */
function normalized(
  components: Component[] = [],
  datasources: Datasource[] = [],
  folders: LocalFolder[] = [],
  translations: { groupPathByUuid?: Record<string, string>; tagNameById?: Record<string, string> } = {},
): NormalizedSchema {
  return {
    ...
    groupPathByUuid: new Map(Object.entries(translations.groupPathByUuid ?? {})),
    tagNameById: new Map(Object.entries(translations.tagNameById ?? {})),
  };
}
...
describe('diffSchema', () => {
  ...
  it('should not show a change when a raw internal_tag_ids entry differs only in JS type', () => {
    ...
    const localComp = { ...makeComponent('page', { title: { type: 'text', pos: 0 } }), internal_tag_ids: [219987616121914] } as Component;
    const remoteComp = { ...makeComponent('page', { title: { type: 'text', pos: 0 } }), internal_tag_ids: ['219987616121914'] } as unknown as Component;
  ...
  it('should not show a change when the target names the tags the base holds as ids', () => {
    const localComp = { ...makeComponent('page', { title: { type: 'text', pos: 0 } }), tags: ['Marketing'] } as unknown as Component;
    const remoteComp = { ...makeComponent('page', { title: { type: 'text', pos: 0 } }), internal_tag_ids: ['7'] } as unknown as Component;

    const result = diffSchema(
      normalized([remoteComp], [], [], { tagNameById: { 7: 'Marketing' } }),
  ...
  it('should show a change when the target names a different tag than the base holds', () => {
    const localComp = { ...makeComponent('page', { title: { type: 'text', pos: 0 } }), tags: ['Editorial'] } as unknown as Component;
    const remoteComp = { ...makeComponent('page', { title: { type: 'text', pos: 0 } }), internal_tag_ids: ['7'] } as unknown as Component;
  ...
  it('should compare a field tag list in name space', () => {
    const localComp = makeComponent('page', { body: { type: 'bloks', pos: 0, component_tag_whitelist: ['Marketing'] } });
    const remoteComp = makeComponent('page', { body: { type: 'bloks', pos: 0, component_tag_whitelist: [7] } });
  ...
  it('should compare a field group list in slug-path space', () => {
    const localComp = makeComponent('page', { body: { type: 'bloks', pos: 0, component_group_whitelist: ['layout'] } });
    const remoteComp = makeComponent('page', { body: { type: 'bloks', pos: 0, component_group_whitelist: ['u1'] } });

    const result = diffSchema(
      normalized([remoteComp], [], [], { groupPathByUuid: { u1: 'layout' } }),
      normalized([localComp]),
    );
