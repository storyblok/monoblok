...
schemaCommand
  ...
  .option(
    "--include-deleted",
    "treat remote-only components as deleted (mirrors `schema push --delete`)",
    false,
  )
  .option(
    "--fail-on-break",
...
Scope:
  ...
  .action(async (entryFile: string, options: SchemaAffectedOptions, command) => {
    ...
    try {
      ...
      loadSpinner.succeed(
        `Found: ${local.components.length} components, ${local.datasources.length} datasources`,
      );
      ...
      if (storiesPath && !(await fileExists(storiesPath))) {
        handleError(
          new CommandError(
            `No local stories found at ${storiesPath}. Run \`storyblok stories pull --space ${space}\` first.`,
          ),
          verbose,
        );
        return;
