...
// rolldown-plugin-dts emits //#region markers per source file when bundling declarations.
// CLI runtime imports always land before the first marker; the source map is always last.
...
// Extracts the type definitions bundled into the CLI declaration file, stripping
// CLI-only runtime imports that appear before the first tsdown #region marker and
// the trailing source map comment. Falls through unchanged for pre-bundled files.
export const extractBundledTypeDefinitions = (content: string) => {
  ...
  return content.slice(declarationStart, declarationEnd).trimEnd();
};
...
export const generateStoryblokTypes = async (options: Pick<GenerateTypesOptions, "path"> = {}) => {
  ...
  try {
    const declarationPath = getPackageJson().types ?? "./dist/index.d.mts";
    const storyblokTypesPath = resolve(__dirname, "..", declarationPath);
