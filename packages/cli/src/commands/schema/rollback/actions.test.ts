...
describe("buildRollbackOps", () => {
  ...
  it("should invert delete to create with before as payload", () => {
    ...
    const changeset: ChangesetData = {
      ...baseChangeset,
      changes: [{ type: "component", name: "footer", action: "delete", before }],
    };
      ...
      changes: [
        {
          ...
          name: "layout/heros",
          action: "create",
          after: { name: "Heros", path: "layout/heros", parentPath: "layout" },
        },
...
describe("executeRollback", () => {
  ...
  it("should send a component create for a create rollback op", async () => {
    ...
    const componentPayload = (capturedComponentBody as { component: Record<string, unknown> })
      .component;
  ...
  it("should recreate a deleted folder and remap a restored component onto its new uuid", async () => {
    ...
    server.use(
      http.post(
        "https://mapi.storyblok.com/v1/spaces/12345/component_groups",
        async ({ request }) => {
    ...
    const ops: RollbackOp[] = [
      {
        ...
        name: "layout/heros",
        action: "create",
        payload: { id: 5, uuid: "old-uuid", name: "Heros", parent_id: null, parent_uuid: null },
      },
      {
        type: "component",
        name: "hero",
        action: "update",
        payload: { name: "hero", component_group_uuid: "old-uuid", schema: {} },
      },
    ];
    const remote: RemoteSchemaData = {
    ...
    expect(
      (capturedGroupBody as { component_group: Record<string, unknown> }).component_group.name,
    ).toBe("Heros");
    expect(
      (capturedUpdateBody as { component: Record<string, unknown> }).component.component_group_uuid,
    ).toBe("new-uuid");
    expect(result.created).toBe(1);
  ...
  it("should delete folders the push created, children-first", async () => {
    ...
    server.use(
      http.delete(
        "https://mapi.storyblok.com/v1/spaces/12345/component_groups/:id",
        ({ params }) => {
