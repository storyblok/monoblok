...
describe("sharedInternalTags resource", () => {
    ...
    server.use(
      http.get("https://mapi.storyblok.com/v1/spaces/12345/shared_internal_tags", ({ request }) => {
        ...
        return HttpResponse.json({
          internal_tags: [{ id: 3, name: "hero", object_type: "asset" }],
        });
      }),
    );
  ...
  it("creates a tag with asset_folder_id in the body", async () => {
      ...
      http.post(
        ...
        async ({ request }) => {
          const body = (await request.json()) as {
            shared_internal_tag: { asset_folder_id: number };
          };
    ...
    const { data } = await client.sharedInternalTags.create({
      body: { shared_internal_tag: { name: "new", object_type: "asset", asset_folder_id: 7 } },
    });
