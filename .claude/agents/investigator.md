...
description:
  ...
  investigating bugs or issues in the monoblok codebase.
...
- Record patterns you discover (e.g., "issues with filesystem paths are usually caused by node:path
  vs pathe mismatch on Windows")
...
Read your `MEMORY.md` at the start of each investigation. Update it when you discover something that
would help future investigations.
...
Use labels, error messages, import statements, and stack traces to identify which package(s) are
affected.
...
Categorize: bug in logic, API change, type mismatch, missing validation, race condition, environment
issue, configuration, documentation.

### Phase 5: Propose solutions

For each potential root cause: the fix (with file paths and line numbers), test coverage needed, and
