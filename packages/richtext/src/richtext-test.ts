...
export type BaseExtensionsReturn = ReturnType<typeof getStoryblokExtensions>;
export type KnownNodeType = keyof BaseExtensionsReturn; // e.g., "paragraph" | "heading" | "image" | ...
/**
 * Renderer Options
 */
export interface StoryblokRichTextOptionsNew {
   ...
   */
  onUnknownNode?: (node: StoryblokRichTextNode) => RichTextSegment[];

  /**
   * Called when a mark has no extension renderer
   */
  onUnknownMark?: (mark: any) => RichTextSegment[];
}
export function formateRichText(richText: StoryblokRichTextNode, options: StoryblokRichTextOptionsNew = {}) {
  const blokExtension = {
  ...
  function renderNode(node: StoryblokRichTextNode): RichTextSegment[] {
    if (node.type === 'text') {
...
function parseDOMSpec(
  spec: DOMOutputSpec,
  nodeType: string,
  fallbackChildren: RichTextSegment[],
): NodeSegment {
