...
import type { Journal, MigrationRun, StoryInverse } from "../src/journal";
...
/**
 * Both backends are held to the same assertions, because the point of the
 * interface is that a rollback cannot tell which one it is reading from.
 */
describe.each([
  ["localJournal", async () => localJournal(await mkdtemp(path.join(tmpdir(), "spike-journal-")))],
  ["mockS3Journal", async () => mockS3Journal("test-bucket", () => {})],
])("%s", (_name, make) => {
  let journal: Journal;
...
describe("localJournal on disk", () => {
  it("should keep the patches out of the file a listing reads", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "spike-journal-"));
