...
describe("diffSchema", () => {
  ...
  const remoteFolders = (
    folders: Array<{ uuid: string; name: string; parent_uuid?: string | null }>,
  ) =>
    new Map(
      folders.map((f) => [f.name, { id: 1, parent_id: null, parent_uuid: null, ...f } as any]),
    );
  ...
  it("should diff component group membership in path space", () => {
    ...
    const diff = diffSchema(local, remote).diffs.find(
      (d) => d.type === "component" && d.name === "hero",
    );
  ...
  it("should not mutate remote component objects across repeated diffs", () => {
    ...
    const second = diffSchema(local, remote).diffs.find(
      (d) => d.type === "component" && d.name === "hero",
    );
