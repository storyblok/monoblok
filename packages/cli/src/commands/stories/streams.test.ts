...
import { createStoriesForLevel, groupStoriesByDepth, readLocalStoriesStream, scanLocalStoryIndex } from './streams';
...
describe('createStoriesForLevel', () => {
  ...
  const fakeCreate = () => mockCreateStory.mockImplementation(async (_spaceId, payload) => ({
    id: ++remoteIdCounter,
    uuid: randomUUID(),
  ...
  describe('skip logic', () => {
    ...
    // both would map to remote-500 causing data loss.
      ...
      await createStoriesForLevel({
        ...
        dryRun: false,
        appendToManifest: noopManifest,
        onStorySuccess(e) { successes.push(e); },
      });
  ...
  describe('validation', () => {
    it('should error when a non-folder story has no component', async () => {
      ...
      await createStoriesForLevel({
        ...
        dryRun: false,
        appendToManifest: noopManifest,
        onStoryError(error) { errors.push(error); },
      });
  ...
  describe('dry-run', () => {
    it('should use fake response in dry-run mode without calling the API', async () => {
      ...
      await createStoriesForLevel({
        ...
        dryRun: true,
        appendToManifest: noopManifest,
        onStorySuccess(_e, remote) { successes.push(remote); },
      });
...
describe('push pipeline: scan -> group -> create level-by-level', () => {
  ...
  it('should create a nested tree with correct parent_id and is_startpage', async () => {
    ...
    writeStory({ id: 5, uuid: randomUUID(), slug: 'recap', name: 'Recap', full_slug: 'blog/2024/recap', is_folder: false, parent_id: subId, content: { _uid: '3', component: 'page' } });
