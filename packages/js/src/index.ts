 ...
 */
function normalizeBridgeOptions(
  options: StoryblokBridgeConfigV2,
): Omit<StoryblokBridgeConfigV2, "language"> {
  ...
  return {
    ...
    ...(resolveRelations !== undefined && {
      resolveRelations: Array.isArray(resolveRelations) ? resolveRelations : [resolveRelations],
    }),
  };
}
...
export const useStoryblokBridge = <T extends StoryblokComponentType<string> = any>(
  ...
  window.storyblokRegisterEvent(() => {
    const sbBridge: StoryblokBridgeV2 = new window.StoryblokBridge(normalizeBridgeOptions(options));
