...
// Emitted by the dedicated declaration build in vite.config.ts. Keep both in sync.
const STORYBLOK_TYPES_FILENAME = "storyblok-types.d.mts";
// rolldown-plugin-dts emits //#region markers per source file and a trailing source map
// comment. Both are bundler bookkeeping and have no place in a user-facing file.
const DECLARATION_REGION_START = "//#region";
const DECLARATION_REGION_MARKER = /^\/\/#(?:end)?region\b.*\n?/gm;
...
const getImportedBindings = (content: string) =>
  [...content.matchAll(IMPORT_STATEMENT)]
...
/**
 * Extracts the Storyblok type definitions from a bundled declaration file.
 *
 * The result is written verbatim into the user's `types/storyblok.d.ts`, so it has to
 * stand on its own: anything the declarations resolve through an import would dangle
 * once the file leaves the CLI package. Imports the declarations do not reference are
 * bundler leftovers and safe to drop; an import they *do* reference means the build
 * leaked something, which is a bug to surface rather than to silently strip.
 *
 * Falls through unchanged for declaration files that were not region-bundled.
 */
export const extractBundledTypeDefinitions = (content: string) => {
  const declarationStart = content.indexOf(DECLARATION_REGION_START);
  if (declarationStart === -1) return content;

  const sourceMapStart = content.indexOf(SOURCE_MAP_START, declarationStart);
  const declarationEnd = sourceMapStart === -1 ? content.length : sourceMapStart;
  const declarations = content
    .slice(declarationStart, declarationEnd)
    .replaceAll(DECLARATION_REGION_MARKER, "")
    .trimEnd();

  const leaked = getImportedBindings(content.slice(0, declarationStart)).filter((binding) =>
    new RegExp(`\\b${binding}\\b`).test(declarations),
  );
  if (leaked.length > 0) {
    throw new CommandError(
      `The packaged CLI declarations resolve ${leaked.join(", ")} through an import, so self-contained Storyblok types cannot be generated. This is a CLI packaging bug, please report it at https://github.com/storyblok/monoblok/issues.`,
    );
  }
...
const getPropertyTypeAnnotation = (property: FieldSchema, prefix?: string, suffix?: string) => {
  ...
  const options =
    optionValues.length > 0 && !property.required ? ["", ...optionValues] : optionValues;
 ...
 */
export const getComponentType = (componentName: string, options: GenerateTypesOptions): string => {
...
const getComponentPropertiesTypeAnnotations = async (
  ...
  return Object.entries(component.schema).reduce(
    async (accPromise, [key, value]) => {
      ...
      if (
        ...
        schema.source === "internal" &&
        schema?.datasource_slug
      ) {
        // Check if the datasource actually exists in spaceData.datasources
        const datasourceExists = spaceData.datasources.some(
          (ds) => ds.slug === schema.datasource_slug,
        );
        if (datasourceExists) {
      ...
      if (propertyType === "multilink") {
        ...
        propertyTypeAnnotation[key].tsType =
          excludedLinktypes.length > 0
            ? `Exclude<${componentType}, ${excludedLinktypes.join(" | ")}>`
            : componentType;
      }
        ...
        if (schema.restrict_components) {
          ...
          if (schema.restrict_type === "groups") {
            if (
                  ...
                  return componentsInGroup.length > 0
                    ? [
                        ...
                        ...componentsInGroup.map((component) =>
                          getComponentType(component.name, options),
                        ),
                      ]
                    : components;
                },
              ...
              propertyTypeAnnotation[key].tsType =
                componentsInGroupWhitelist.length > 0
                  ? `(${componentsInGroupWhitelist.join(" | ")})[]`
                  : `never[]`;
            }
          } else if (schema.restrict_type === "tags") {
            // Components restricted by tags
            if (
              Array.isArray(schema.component_tag_whitelist) &&
              schema.component_tag_whitelist.length > 0
            ) {
            ...
            if (
              Array.isArray(schema.component_whitelist) &&
              schema.component_whitelist.length > 0
            ) {
...
export const generateTypes = async (
  ...
  try {
    ...
    const componentsSchema = spaceData.components.map(async (component) => {
      ...
      const componentPropertiesTypeAnnotations = await getComponentPropertiesTypeAnnotations(
        component,
        options,
        spaceData,
        customFieldsParser,
    ...
    const datasourcesSchema = spaceData.datasources.map(async (datasource) => {
      ...
      const filtered = datasource.entries?.filter((d) => d.value).map((d) => d.value!);
    ...
    const resolvedDatasourcesSchema = resolvedDatasourcesSchemaWithNulls.filter(
      (s): s is JSONSchema => s !== null,
    );
    ...
    const schemas = [...resolvedComponentsSchema, ...resolvedDatasourcesSchema, contentTypeSchema];
    ...
    const result = await Promise.all(
      schemas.map(async (schema) => {
        ...
        const kind = componentTitles.has(title)
          ? "component"
          : datasourceTitles.has(title)
            ? "datasource"
            : "type";
    ...
    if (options.separateFiles) {
      ...
      const contentTypesFile = createContentTypesFile(contentTypeBloks, typeDefs);
      if (contentTypesFile) {
...
export const generateStoryblokTypes = async (options: Pick<GenerateTypesOptions, "path"> = {}) => {
  ...
  try {
    const storyblokTypesPath = resolve(__dirname, STORYBLOK_TYPES_FILENAME);
    const storyblokTypesContent = extractBundledTypeDefinitions(
      readFileSync(storyblokTypesPath, "utf-8"),
    );

    // Define the content of the d.ts file. Trailing newline to match the component
    // declarations and to keep the file well-formed for editors and formatters.
    const typeDefs = `${[
    ...
    // A packaging problem is not a filesystem problem: handleFileSystemError would
    // collapse it into "An unknown error occurred" and lose the actionable message.
