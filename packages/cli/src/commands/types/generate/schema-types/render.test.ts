...
describe("renderSchemaTypes", () => {
  it("emits the definition types and the shared surface", () => {
    ...
    expect(output).toContain(
      "import type { BlockContent, MapiStory as InferStoryMapi, Story as InferStory } from '@storyblok/schema';",
    );
    expect(output).not.toContain("InferSchema");
    expect(output).toContain("export type HeroBlockDefinition = {");
    expect(output).toContain(
      "export type Blocks = HeroBlockDefinition | TeaserListBlockDefinition;",
    );
  ...
  it("renames internal references consistently with prefixed declarations", () => {
    ...
    expect(output).toContain(
      "export type StoryblokSchema = { blocks: StoryblokBlocks; fieldPlugins: StoryblokFieldPlugins };",
    );
    expect(output).toContain(
      "export type StoryblokBlock<TName extends StoryblokBlocks['name']> = BlockContent<Extract<StoryblokBlocks, { name: TName }>, StoryblokBlocks, StoryblokFieldPlugins>;",
    );
  ...
  it("derives FieldPlugins from a defineSchema result", () => {
    ...
    expect(output).toContain(
      "export type FieldPlugins = InferSchema<typeof userSchema>['fieldPlugins'];",
    );
  });
  ...
  it("records the space in the generated header", () => {
    ...
    expect(
      output.startsWith("// This file was generated by the Storyblok CLI. Do not edit by hand."),
    ).toBe(true);
...
describe("toRelativeImport", () => {
  it("builds a posix relative specifier with a javascript extension", () => {
    expect(toRelativeImport("/p/.storyblok/types/1", "/p/.storyblok/schema/schema.ts")).toBe(
      "../../schema/schema.js",
    );
  });
...
describe("renderSeparateFiles", () => {
  ...
  it("emits no extension-less relative import in any file, in either mode", () => {
    const options = {
      ...
      fieldPlugins: {
        ...
        modulePath: "/abs/plugins.ts",
        fieldTypes: ["colorpicker"],
      },
    ...
    const specifiers = emitted.flatMap((content) =>
  ...
  it("writes one definition per block file and imports them in the main file", () => {
    ...
    expect(files.get("storyblok-schema.d.ts")).toContain(
      "import type { HeroBlockDefinition } from './blocks/hero.js';",
    );
    expect(files.get("storyblok-schema.d.ts")).toContain(
      "import type { TeaserListBlockDefinition } from './blocks/teaser-list.js';",
    );
    expect(files.get("storyblok-schema.d.ts")).toContain(
      "export type Blocks = HeroBlockDefinition | TeaserListBlockDefinition;",
    );
  ...
  it("uses the safe name in both the block file and the main file import", () => {
    ...
    expect(files.get("storyblok-schema.d.ts")).toContain(
      "import type { _2ColBlockDefinition } from './blocks/2-col.js';",
    );
