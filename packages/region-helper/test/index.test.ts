...
describe("getRegionBaseUrl https", () => {
  ...
  it("should return `eu url` region", () => {
    expect(getRegionBaseUrl(EU_CODE)).toBe(`${httpsProtocol}://${EU_API_DOMAIN}`);
    expect(getRegionBaseUrl(EU_CODE, "https")).toBe(`${httpsProtocol}://${EU_API_DOMAIN}`);
  });

  it("should return `us url` region", () => {
    expect(getRegionBaseUrl(US_CODE)).toBe(`${httpsProtocol}://${US_API_DOMAIN}`);
  });

  it("should return `cn url` region", () => {
    expect(getRegionBaseUrl(CN_CODE)).toBe(`${httpsProtocol}://${CN_API_DOMAIN}`);
  });

  it("should return `ca url` region", () => {
    expect(getRegionBaseUrl(CA_CODE)).toBe(`${httpsProtocol}://${CA_API_DOMAIN}`);
  });

  it("should return `ap url` region", () => {
    expect(getRegionBaseUrl(AP_CODE)).toBe(`${httpsProtocol}://${AP_API_DOMAIN}`);
  });
});
...
describe("getRegionBaseUrl http", () => {
  ...
  it("should return `eu url` region", () => {
    expect(getRegionBaseUrl(EU_CODE, httpProtocol)).toBe(`${httpProtocol}://${EU_API_DOMAIN}`);
  });

  it("should return `us url` region", () => {
    expect(getRegionBaseUrl(US_CODE, httpProtocol)).toBe(`${httpProtocol}://${US_API_DOMAIN}`);
  });

  it("should return `cn url` region", () => {
    expect(getRegionBaseUrl(CN_CODE, httpProtocol)).toBe(`${httpProtocol}://${CN_API_DOMAIN}`);
  });

  it("should return `ca url` region", () => {
    expect(getRegionBaseUrl(CA_CODE, httpProtocol)).toBe(`${httpProtocol}://${CA_API_DOMAIN}`);
  });

  it("should return `ap url` region", () => {
    expect(getRegionBaseUrl(AP_CODE, httpProtocol)).toBe(`${httpProtocol}://${AP_API_DOMAIN}`);
  });
});
