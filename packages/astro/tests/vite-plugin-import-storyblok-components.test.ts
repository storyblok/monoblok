...
import {
  ...
  generateModuleCode,
} from "../src/vite-plugins/vite-plugin-import-storyblok-components";
...
describe("vite-plugin-import-storyblok-components", () => {
  describe("createComponentRegistrationParts", () => {
    it("generates structured parts with wrapper for TDZ avoidance", () => {
      ...
      expect(parts.importStatement).toBe(
        `import __Card_component__ from '/src/components/Card.astro';`,
      );
      expect(parts.wrapperDefinition).toBe(
        `const __Card_wrapper__ = { get default() { return __Card_component__; } };`,
      );
  ...
  describe("generateModuleCode", () => {
    ...
    it("generates valid module code with manual components", () => {
      ...
      expect(code).toContain(
        `const __hero_wrapper__ = { get default() { return __hero_component__; } }`,
      );
    ...
    it("generates valid module code with fallback component", () => {
      ...
      expect(code).toContain(
        `import __FallbackComponent_component__ from '@storyblok/astro/FallbackComponent.astro'`,
      );
      expect(code).toContain(
        `const __FallbackComponent_wrapper__ = { get default() { return __FallbackComponent_component__; } }`,
      );
      expect(code).toContain(
        `registerComponent('FallbackComponent', __FallbackComponent_wrapper__)`,
      );
    });
