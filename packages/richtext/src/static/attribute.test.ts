...
describe("processAttrs", () => {
  ...
  describe("invalid and skipped values", () => {
    ...
    it("excludes all attributes in EXCLUDED_ATTRS", () => {
      const excludedAttrs = Array.from(EXCLUDED_ATTRS).reduce(
        (acc, attr) => {
          acc[attr] = "test";
          return acc;
        },
  ...
  describe("default attribute mapping", () => {
    it("excludes fallbackImage from output", () => {
      expect(
        processAttrs("emoji", { fallbackImage: "https://cdn.example.com/rocket.png" }),
      ).toEqual({});
    });
  ...
  describe("extended attribute mapping", () => {
    ...
    it("applies multiple extended mappings", () => {
      expect(
        processAttrs("paragraph", { foo: 1, bar: 2 }, { foo: "data-foo", bar: "data-bar" }),
      ).toEqual({
        "data-foo": 1,
        "data-bar": 2,
      });
