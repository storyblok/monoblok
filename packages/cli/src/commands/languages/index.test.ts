  ...
  describe("pull", () => {
    ...
    describe("default mode", () => {
      it("should prompt the user if operation was sucessfull", async () => {
        ...
        expect(saveLanguagesToFile).toHaveBeenCalledWith(
          "12345",
          mockResponse,
        ...
        const expectedPath = relative(
          process.cwd(),
          join(resolveCommandPath("languages", "12345"), "languages.12345.json"),
        );
        expect(konsola.ok).toHaveBeenCalledWith(
          `Languages schema downloaded successfully at ${chalk.hex(colorPalette.PRIMARY)(expectedPath)}`,
          true,
        );
      });
    ...
    describe("--path option", () => {
      it("should save the file at the provided path", async () => {
        ...
        const expectedPath = join(
          resolveCommandPath("languages", "12345", "/tmp"),
          "languages.12345.json",
        );
        expect(konsola.ok).toHaveBeenCalledWith(
          `Languages schema downloaded successfully at ${chalk.hex(colorPalette.PRIMARY)(expectedPath)}`,
          true,
    ...
    describe("--filename option", () => {
      it("should save the file with the provided filename", async () => {
        ...
        vi.mocked(fetchLanguages).mockResolvedValue(mockResponse);
        await languagesCommand.parseAsync([
          "node",
          "test",
          "pull",
          "--space",
        ...
        const expectedPath = relative(
          process.cwd(),
          join(resolveCommandPath("languages", "12345"), "custom-languages.12345.json"),
        );
        expect(konsola.ok).toHaveBeenCalledWith(
          `Languages schema downloaded successfully at ${chalk.hex(colorPalette.PRIMARY)(expectedPath)}`,
          true,
    ...
    describe("--suffix option", () => {
      it("should save the file with the provided suffix", async () => {
        ...
        const expectedPath = relative(
          process.cwd(),
          join(resolveCommandPath("languages", "12345"), "languages.custom-suffix.json"),
        );
        expect(konsola.ok).toHaveBeenCalledWith(
          `Languages schema downloaded successfully at ${chalk.hex(colorPalette.PRIMARY)(expectedPath)}`,
          true,
