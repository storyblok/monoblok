...
describe("generateComponentFile", () => {
  ...
  it("should escape backslashes and newlines in field values so the output round-trips", () => {
    ...
    expect(result).toContain(
      String.raw`description: 'The identifier can be found here:\nhttps://vimeo.com/<id>',`,
    );
  ...
  it("should combine the block's own folder and a whitelist folder into one import at the right depth", () => {
    ...
    const result = generateComponentFile(
      ...
      new Map([
        ["uuid-heros", "herosFolder"],
        ["uuid-marketing", "marketingFolder"],
      ]),
    );
...
describe("generateDatasourceFile", () => {
  ...
  it("should omit empty-array fields such as dimensions", () => {
    const datasource = {
      id: 1,
      name: "Categories",
      slug: "categories",
  ...
  it("should keep non-empty dimensions", () => {
    const datasource = {
      id: 1,
      name: "Categories",
      slug: "categories",
      dimensions: [{ name: "de", entry_value: "de" }],
    };
...
describe("resolveVarNames", () => {
  it("leaves distinct names untouched", () => {
    expect(resolveVarNames(["page", "hero"], componentVarName)).toEqual(["pageBlock", "heroBlock"]);
  });

  it("appends a numeric suffix when sanitized names collide", () => {
    expect(resolveVarNames(["Colors & Sizes", "Colors / Sizes"], datasourceVarName)).toEqual([
      "colorsSizesDatasource",
      "colorsSizesDatasource2",
    ]);
  });
});
...
describe("resolveFileNames", () => {
  ...
  it("appends a -N suffix when kebab-collapsed names collide", () => {
    ...
    expect(resolveFileNames(["hero-cta", "hero-cta", "hero-cta"])).toEqual([
      "hero-cta",
      "hero-cta-2",
      "hero-cta-3",
    ]);
  ...
  it("scopes uniqueness per directory so different dirs may share a name", () => {
    ...
    expect(resolveFileNames(["hero", "hero", "hero"], ["group-a", "group-b", "group-a"])).toEqual([
      "hero",
      "hero",
      "hero-2",
    ]);
...
describe("generateSchemaFile with colliding datasources", () => {
  it("imports and keys each datasource under a unique identifier", () => {
    ...
    expect(result).toContain(
      "import { colorsSizesDatasource2 } from './datasources/colors-slash-sizes';",
    );
