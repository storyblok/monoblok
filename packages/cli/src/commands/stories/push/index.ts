...
const pushCmd = storiesCommand
  ...
  .option(
    "--cleanup",
    "delete local stories after a successful push (note: does not cleanup manifests)",
  )
...
pushCmd.action(async (options, command) => {
  ...
  try {
    const manifestFile = join(
      resolveCommandPath(directories.stories, fromSpace, basePath),
      "manifest.jsonl",
    );
    const manifest = await loadManifest(manifestFile);
    const assetManifestFile = join(
    ...
    const schemaIssues = await collectSchemaIssues({
    ...
    for (const level of levels) {
      await createStoriesForLevel({
        ...
        onStorySuccess(entry, remoteStory) {
          if (!entry.uuid) {
            throw new Error(
              `Local story file "${entry.filename}" is missing a "uuid" field. Re-pull the story or add the uuid manually.`,
            );
          }
          if (!remoteStory.uuid) {
            throw new Error(
              `Storyblok API returned a story without a uuid for slug "${entry.slug}".`,
            );
          }
