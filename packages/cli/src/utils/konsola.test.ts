...
describe("konsola", () => {
  describe("title", () => {
    it("should prompt a title message", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        formatHeader(chalk.bgHex("#45bfb9").bold.white(` This is a test title `)),
      );
    });
  });
  describe("warn", () => {
    it("should prompt a warning message", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        `${chalk.yellow("⚠️ ")} This is a test warning message`,
      );
    });
  ...
  describe("success", () => {
    it("should prompt an success message", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        `${chalk.green("✔")} Component A created succesfully`,
      );
    });
