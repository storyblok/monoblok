...
| `--target <target>` | Push destination: `space`, `shared`, or `auto` (routes each local asset to its own space or library). A single-asset push accepts only `space` or `shared`. | `space` for a single asset, `auto` for a bulk push |
