...
describe("images-optimization", () => {
  ...
  it("should not add width or height to the src if both are at 0", async () => {
    ...
    expect(consoleWarnSpy).toBeCalledWith(
      "[StoryblokRichText] - Width and height values cannot both be 0",
    );
  });
  ...
  it("should not add width to the src if width is not a number", async () => {
    ...
    expect(consoleWarnSpy).toBeCalledWith(
      "[StoryblokRichText] - Width value must be a number greater than or equal to 0",
    );
  ...
  it("should not add height to the src if height is not a number", async () => {
    ...
    optimizeImage(src, { width: 800, height: "600" });
    expect(consoleWarnSpy).toBeCalledWith(
      "[StoryblokRichText] - Height value must be a number greater than or equal to 0",
    );
    consoleWarnSpy.mockRestore();
  });
  ...
  it("should not add blur filter if value is not a number", async () => {
    ...
    expect(consoleWarnSpy).toBeCalledWith(
      "[StoryblokRichText] - Blur value must be a number between 0 and 100 (inclusive)",
    );
    consoleWarnSpy.mockRestore();
  });
  ...
  it("should not add brightness filter if value is not a number", async () => {
    ...
    expect(consoleWarnSpy).toBeCalledWith(
      "[StoryblokRichText] - Brightness value must be a number between 0 and 100 (inclusive)",
    );
    consoleWarnSpy.mockRestore();
  });
  ...
  it("should not add quality filter if value is not a number", async () => {
    ...
    expect(consoleWarnSpy).toBeCalledWith(
      "[StoryblokRichText] - Quality value must be a number between 0 and 100 (inclusive)",
    );
    consoleWarnSpy.mockRestore();
  });
  ...
  it("should add multiple filters if provided", async () => {
    ...
    const filters = {
      ...
      rotate: 90,
      format: "webp",
    } satisfies StoryblokRichTextImageOptions["filters"];
    const { src: resultSrc } = optimizeImage(src, { filters });
    expect(resultSrc).toBe(
      `${src}/m/filters:blur(5):quality(80):brightness(0.5):fill(transparent):grayscale():rotate(90):format(webp)`,
    );
  });
  ...
  it("should add srcset attribute if provided", async () => {
    ...
    expect(attrs).toEqual({
      srcset:
        "https://a.storyblok.com/f/279818/710x528/c53330ed26/tresjs-doge.jpg/m/400x0/ 400w, https://a.storyblok.com/f/279818/710x528/c53330ed26/tresjs-doge.jpg/m/800x0/ 800w, https://a.storyblok.com/f/279818/710x528/c53330ed26/tresjs-doge.jpg/m/1200x0/ 1200w",
    });
  });
  ...
  it("should add srcset attribute with width and height if provided as an array of arrays", async () => {
    ...
    const srcset = [
      ...
      [800, 600],
      [1200, 900],
    ] satisfies StoryblokRichTextImageOptions["srcset"];
    const { attrs } = optimizeImage(src, { srcset });
    expect(attrs).toEqual({
      srcset:
        "https://a.storyblok.com/f/279818/710x528/c53330ed26/tresjs-doge.jpg/m/400x300/ 400w, https://a.storyblok.com/f/279818/710x528/c53330ed26/tresjs-doge.jpg/m/800x600/ 800w, https://a.storyblok.com/f/279818/710x528/c53330ed26/tresjs-doge.jpg/m/1200x900/ 1200w",
    });
  });
