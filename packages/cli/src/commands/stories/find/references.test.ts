...
function makeComponent(name: string, schema: Record<string, Record<string, unknown>>): Component {
  return { id: 1, name, schema, created_at: '', updated_at: '', is_root: false, is_nestable: true } as Component;
}
...
describe('extractReferences', () => {
  ...
  it('ignores non-story multilink', () => {
    const story = makeStory({
      component: 'page',
      link: { fieldtype: 'multilink', linktype: 'url', url: 'https://example.com', cached_url: 'https://example.com' },
    });
    const refs = extractReferences(story, emptyRelMap);
