...
export async function writeSchemaFiles(
  ...
  for (const { component, varName, fileName, segments } of resolvedComponents) {
    ...
    const folder = component.component_group_uuid
      ? folderByUuid.get(component.component_group_uuid)
      : undefined;
  ...
  await writeFileWithDirs(
    schemaPath,
    generateSchemaFile(resolvedComponents, resolvedDatasources, resolvedFolders),
  );
