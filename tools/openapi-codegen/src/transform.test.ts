...
describe("transformGeneratedFile", () => {
  ...
  it("should unwrap a dotted envelope path", () => {
    const source =
      "export type CreateAssetFolderData = { body: { asset_folder: { name: string } } };";
  ...
  it("should convert an interface with extends to an intersection", () => {
    ...
    const { output } = run(source, [
      { source: "Base", emitAs: "Base" },
      { source: "Tag", emitAs: "Tag" },
    ]);
  ...
  it("should emit secondary aliases of the same source as references to the primary", () => {
    ...
    const { output, emitted } = run(source, [
  ...
  it("should prefix the output with the generated-file header", () => {
    const { output } = run("export type Story = { id: number };", [
      { source: "Story", emitAs: "Story" },
    ]);

    expect(
      output.startsWith("// Generated by @storyblok/openapi-codegen. Do not edit by hand."),
    ).toBe(true);
  });
  ...
  it("should throw when a requested source is absent from the input", () => {
    ...
    expect(() => run(source, [{ source: "Missing", emitAs: "Missing" }])).toThrow(
      'Upstream type "Missing" not found',
    );
  });
  ...
  it("should throw when an unwrap segment does not exist", () => {
    ...
    expect(() =>
      run(source, [{ source: "UpdateStoryRequest", emitAs: "StoryCreate", unwrap: "nope" }]),
