...
export function convertAttributesInElement(
  ...
  const children = React.Children.map(
    ...
    (child) => convertAttributesInElement(child),
  );
...
function App() {
  ...
  const options: StoryblokRichTextOptions<ReactElement> = {
    ...
    resolvers: {
      [BlockTypes.LIST_ITEM]: (
        ...
        ctx: StoryblokRichTextContext<ReactElement>,
      ) => {
  ...
  const html = richTextResolver<ReactElement>(options).render(story.content.richtext);
