...
describe("detectRenames", () => {
  ...
  it("does not match when types differ", () => {
    ...
    expect(result.unmatchedAdded).toEqual([
      { field: "title_new", type: "number", required: false },
    ]);
  });
  ...
  it("leaves unmatched fields when no type match is found", () => {
    ...
    expect(result.unmatchedAdded).toEqual([
      { field: "new_field", type: "markdown", required: false },
    ]);
  });
  ...
  it("should return breaking changes for updated components", () => {
    const local: SchemaData = {
      components: [
        {
          name: "hero",
          schema: { author: { type: "text", pos: 0 }, rating: { type: "number", pos: 1 } },
        },
      ] as any,
      folders: [],
      datasources: [],
    ...
    const remote: RemoteSchemaData = {
      components: new Map([
        [
          ...
          {
            name: "hero",
            schema: { author_name: { type: "text", pos: 0 }, rating: { type: "text", pos: 1 } },
          } as any,
        ],
      ]),
  ...
  it("should skip components with no breaking changes", () => {
    const local: SchemaData = {
      components: [
        {
          name: "hero",
          schema: { title: { type: "text", pos: 0 }, subtitle: { type: "text", pos: 1 } },
        },
      ] as any,
      folders: [],
      datasources: [],
