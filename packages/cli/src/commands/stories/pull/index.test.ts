        ...
        const matchesParam = (key: string, value: unknown): boolean => {
          ...
          if (value !== null && typeof value === "object") {
            return Object.entries(value).every(([childKey, childValue]) =>
              matchesParam(`${key}[${childKey}]`, childValue),
            );
          }
          return url.searchParams.get(key) === String(value);
        };
        const matchesAllParams = Object.entries(params).every(([key, value]) =>
...
describe("stories pull command", () => {
  ...
  it("should pull all stories", async () => {
    ...
    const reportFile = Object.entries(vol.toJSON()).find(([filename]) =>
      filename.includes("reports/12345/storyblok-stories-pull-"),
    )?.[1];
