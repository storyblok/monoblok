...
describe("throttleQueueManager", () => {
  ...
  describe("request execution", () => {
    it("should pass arguments to throttled request function", async () => {
      ...
      expect(mockThrottledRequest).toHaveBeenCalledWith("get", "/test", { version: "draft" });
    });
  ...
  describe("common rate limit scenarios", () => {
    ...
    it("should handle typical uncached request rate limits", async () => {
      ...
      await manager
        .execute(50, "get", "/draft-small", { version: "draft", per_page: 25 })
        .catch(() => {});

      // Medium listing - 15 req/s
      await manager
        .execute(15, "get", "/draft-medium", { version: "draft", per_page: 40 })
        .catch(() => {});

      // Large listing - 10 req/s
      await manager
        .execute(10, "get", "/draft-large", { version: "draft", per_page: 60 })
        .catch(() => {});

      // Very large listing - 6 req/s
      await manager
        .execute(6, "get", "/draft-xlarge", { version: "draft", per_page: 100 })
        .catch(() => {});
