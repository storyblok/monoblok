...
- **The deprecated overload keeps one piece of per-visitor state alive** until 2.0, so "holds no per-visitor state" is true of the supported API but not yet of the implementation. The cleanup comment names all four pieces to remove together.
- **Two runtime behavior changes ride along, both turning a silent failure into a loud one.** They are not type-level, so they can break code that still compiles, and they belong in the release notes rather than only here.
  - `track(goal, secondArgument)` throws a `TypeError` when the second argument is present but is neither a string nor an object. That value is nearly always a `visitorId` that went missing, such as an unset cookie reaching `track(goal, cookies.get('sb_vid')?.value)`. Letting it through would route the call to the deprecated path and attribute the conversion to whichever visitor the instance resolved last, which on the now-recommended module-scope instance is a different visitor's request. The cost is that forwarding an optional props bag, `track(goal, maybeProps)`, now throws when the bag is absent, so callers doing that pass no second argument instead. Loud breakage of a deprecated form beats silent cross-visitor corruption, which is invisible in the sink. Arity is what separates the two cases, since a default parameter value applies to an explicitly passed `undefined` as well as to an omitted argument.
  - `fetchAdapter` resolves its url at construction and throws on a relative path that it cannot resolve, where 1.x constructed fine and then failed on every delivery into `onError`. The tradeoff is that eager validation and a module-scope adapter shared between server and client code pull in opposite directions: `fetchAdapter('/api/events')` in such a module now throws on the server even when only the browser delivers through it. Deferring the throw would restore the silent per-delivery failure, so the check stays eager and the JSDoc names the ways out, which are `baseUrl`, an absolute url, building the adapter where it is used, or `beaconAdapter` on the client.
