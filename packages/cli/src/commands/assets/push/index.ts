...
pushCmd.action(async (assetInput, options, command) => {
  ...
  const manifestFile = join(
    resolveCommandPath(directories.assets, fromSpace, basePath),
    "manifest.jsonl",
  );
  const folderManifestFile = join(
    resolveCommandPath(directories.assets, fromSpace, basePath),
    "folders",
    "manifest.jsonl",
  );
  ...
  try {
    ...
    const cleanupAssetTransport =
      options.cleanup && !options.dryRun ? makeCleanupAssetFSTransport() : () => Promise.resolve();
    ...
    const hasUpdatedAssets = maps.assets
      .values()
