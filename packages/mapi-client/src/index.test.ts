...
describe("createManagementApiClient - HTTP requests", () => {
  it("should retrieve spaces successfully", async () => {
    server.use(
      http.get("https://mapi.storyblok.com/v1/spaces", () =>
        HttpResponse.json({ spaces: [{ id: 1, name: "Test Space" }] }),
      ),
    );
  ...
  it("should keep each client instance independent", async () => {
    ...
    const client1 = createManagementApiClient({
      personalAccessToken: "token-a",
      spaceId: 123,
      region: "eu",
      rateLimit: false,
    });
    const client2 = createManagementApiClient({
...
describe("createManagementApiClient - HTTP method helpers", () => {
  it("client.get() should send a GET request and return data", async () => {
    server.use(
      http.get("https://mapi.storyblok.com/v1/spaces/123/custom", () =>
