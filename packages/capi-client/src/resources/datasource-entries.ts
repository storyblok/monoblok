...
export function createDatasourceEntriesResource(deps: ResourceDeps) {
  ...
  return {
    list: async <ThrowOnError extends boolean = false>(
      options: {
        query?: DatasourceEntriesListData["query"];
        signal?: AbortSignal;
      ...
      return requestWithCache<DatasourceEntriesListResponses[200], ThrowOnError>(
        "GET",
        requestPath,
        query,
        (requestQuery: Record<string, unknown>) => {
          return throttleManager.execute(requestPath, requestQuery, () =>
            asApiResponse<DatasourceEntriesListResponses[200], ThrowOnError>(
              listDatasourceEntriesApi({
                client,
                query: requestQuery,
                signal,
