...
describe("sharedAssets resource", () => {
    ...
    server.use(
      http.get("https://mapi.storyblok.com/v1/spaces/12345/shared_assets", ({ request }) => {
        ...
        return HttpResponse.json(
          {
            assets: [
              { id: 1, filename: "https://a.storyblok.com/f/12345/x.png", asset_folder_id: 7 },
            ],
          },
  ...
  it("creates a shared asset via sign -> S3 -> finish flow", async () => {
    server.use(
      http.post("https://mapi.storyblok.com/v1/spaces/12345/shared_assets", () =>
        HttpResponse.json({
          id: 55,
          post_url: "https://s3.test/upload",
          fields: { key: "g/1/x.png", "Content-Type": "image/png" },
        }),
      ),
    ...
    const asset = await client.sharedAssets.create({
      body: { short_filename: "x.png", asset_folder_id: 7 },
  ...
  it("re-signs an existing shared asset when replacing its file", async () => {
    ...
    server.use(
      http.post("https://mapi.storyblok.com/v1/spaces/12345/shared_assets", ({ request }) => {
        ...
        return HttpResponse.json({
          id: 77,
          post_url: "https://s3.test/upload",
          fields: { key: "g/1/y.png", "Content-Type": "image/png" },
        });
      }),
      http.post("https://s3.test/upload", () => new HttpResponse(null, { status: 204 })),
    ...
    await client.sharedAssets.update(77, {
      body: { asset: { title: "New title" }, short_filename: "y.png" },
      file: new ArrayBuffer(4),
    });
