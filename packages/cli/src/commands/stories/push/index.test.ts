...
const server = setupServer(
  http.get("https://mapi.storyblok.com/v1/spaces/:spaceId/stories/:storyId", () =>
...
const preconditions = {
  ...
  canLoadStories(stories: MockStory[], space = DEFAULT_SPACE, basePath?: string) {
    ...
    vol.fromJSON(
      Object.fromEntries(
        stories.map((s) => [join(storiesDir, `${s.slug}_${s.uuid}.json`), JSON.stringify(s)]),
      ),
    );
  },
    ...
    server.use(
      http.post(`https://mapi.storyblok.com/v1/spaces/${space}/stories`, () => {
        return HttpResponse.json(
          { message: "Failed to create placeholder story" },
          { status: 500 },
        );
      }),
    );
...
describe("stories push command", () => {
  ...
  describe("first-time push", () => {
    it("should push stories with mapped references", async () => {
      ...
      expect(manifestEntries).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            old_id: storyA.uuid,
            new_id: storyARemote.uuid,
            created_at: expect.any(String),
          }),
          expect.objectContaining({
            old_id: storyB.uuid,
            new_id: storyBRemote.uuid,
            created_at: expect.any(String),
          }),
          expect.objectContaining({
            old_id: storyC.uuid,
            new_id: storyCRemote.uuid,
            created_at: expect.any(String),
          }),
          expect.objectContaining({
            old_id: storyD.uuid,
            new_id: storyDRemote.uuid,
            created_at: expect.any(String),
          }),
          expect.objectContaining({
            old_id: storyE.uuid,
            new_id: storyERemote.uuid,
            created_at: expect.any(String),
          }),
        ]),
      );
      ...
      expect(logFile).toContain(
        '"creationResults":{"total":5,"succeeded":5,"skipped":0,"failed":0}',
      );
    ...
    it("should send parent_id and is_startpage during creation when parent is resolved", async () => {
      ...
      const folderCall = calls.find(
        (c: unknown[]) => (c[1] as { story: { slug: string } }).story.slug === "lang-a",
      )!;
      ...
      const startpageCall = calls.find(
        (c: unknown[]) => (c[1] as { story: { slug: string } }).story.slug === "home",
      )!;
  ...
  describe("error handling", () => {
    ...
    it("should handle errors when writing to the manifest fails", async () => {
      ...
      const remoteStories = preconditions.canCreateStories([storyA]);
      preconditions.canUpdateStories(remoteStories);
      const appendFileError = Object.assign(new Error("Manifest append failed"), {
        code: "EACCES",
      });
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining(`story-a (uuid: `));
      // UI
      expect(console.info).toHaveBeenCalledWith(
    ...
    it("should abort with a hard error when a story references a missing component schema", async () => {
      ...
      preconditions.canLoadStories([storyA]);
      preconditions.canLoadComponents([makeMockComponent({ name: "article" })]);
    ...
    it("should report both missing schemas and drift fields together", async () => {
      ...
      preconditions.canLoadStories([storyA, storyB]);
      preconditions.canLoadComponents([makeMockComponent({ name: "page", schema: {} })]);
    ...
    it("should not delete local stories when validation fails even if --cleanup is passed", async () => {
      ...
      const hasStoryFile = Object.keys(files).some(
        (f) => stripDriveLetter(f) === stripDriveLetter(storyFilePath),
      );
    ...
    it("should warn once per plugin name when story content carries a custom plugin payload", async () => {
      ...
      const warnCalls = (console.warn as unknown as ReturnType<typeof vi.fn>).mock.calls.filter(
        (call) => typeof call[0] === "string" && call[0].includes("official-colorpicker"),
      );
      expect(warnCalls).toHaveLength(1);
      const logFile = getLogFileContents(LOG_PREFIX);
      expect(logFile).toContain(
        'The custom plugin \\"official-colorpicker\\" may contain references that require manual updates.',
      );
    });
    ...
    it("should handle errors when reading local stories fails", async () => {
      ...
      // Logging
      const logFile = getLogFileContents(LOG_PREFIX);
      expect(logFile).toContain("Expected property name or '}' in JSON at position 1");
      // UI — deferred, grouped summary (no inline console.error during streaming)
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("Failed stories (1):"),
        expect.anything(),
      );
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining("story-a.json"));
      // UI
      expect(console.info).toHaveBeenCalledWith(
        expect.stringContaining("Push results: 1 story pushed, 1 story failed"),
      );
