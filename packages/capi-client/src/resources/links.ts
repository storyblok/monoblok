...
import type {
  ListData as LinksListData,
  ListResponses as LinksListResponses,
} from "../generated/links/types.gen";
...
export function createLinksResource(deps: ResourceDeps) {
  ...
  return {
    list: async <ThrowOnError extends boolean = false>(
      ...
      return requestWithCache<LinksListResponses[200], ThrowOnError>(
        "GET",
        requestPath,
        query,
        (requestQuery: Record<string, unknown>) => {
          return throttleManager.execute(requestPath, requestQuery, () =>
            asApiResponse<LinksListResponses[200], ThrowOnError>(
              listLinks({
                client,
                query: requestQuery,
                signal,
