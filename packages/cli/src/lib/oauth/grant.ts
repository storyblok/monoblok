...
export const introspectGrant = async (
  region: RegionCode,
  accessToken: string,
): Promise<GrantIntrospection> => {
  let body: { grant?: GrantIntrospection };
  try {
    body = await customFetch<{ grant?: GrantIntrospection }>(
      `${getStoryblokUrl(region)}/oauth/grant`,
      {
    ...
    if (error instanceof FetchError) {
      throw new CommandError(
        `Grant introspection failed (${error.response.status} ${error.response.statusText}).`,
      );
    }
    throw error;
  }
