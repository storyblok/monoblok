...
const DEFAULT_FILENAME_PREFIX = 'storyblok'
...
export const saveTypesToFile = async (space: string, typedefString: string, options: SaveTypesOptions) => {
  const { filename = DEFAULT_FILENAME_PREFIX, path } = options;
  ...
  try {
    await saveToFile(join(resolvedPath, `${filename}-components.d.ts`), typedefString);
  }
  catch (error) {
