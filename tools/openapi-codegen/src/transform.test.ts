...
describe("deduplicateGeneratedUnions", () => {
  it("should deduplicate unions across a complete generated file", () => {
    const source = [
      "// Generated by hey-api",
      "export type Content = { [key: string]: string | null | string | Source | Source };",
      "export interface Nested { value: number | number | boolean; }",
    ].join("\n");

    const output = deduplicateGeneratedUnions(source);

    expect(squish(output)).toContain("[key: string]: string | null | Source }");
    expect(squish(output)).toContain("value: number | boolean;");
  });

  it("should preserve unchanged source text when no duplicates exist", () => {
    const source = "export type Content = { value: string | null };\n";

    expect(deduplicateGeneratedUnions(source)).toBe(source);
  });
});
...
describe("transformGeneratedFile", () => {
  ...
  it("should preserve the first duplicate member and its documentation", () => {
    const source = ["export type Content = { value: /** documented */ string | string };"].join(
      "\n",
    );

    const { output } = run(source, [{ source: "Content", emitAs: "Content" }]);

    expect(squish(output)).toContain("value: /** documented */ string;");
  });
