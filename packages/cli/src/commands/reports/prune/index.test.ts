...
describe("reports prune command", () => {
  ...
  it("should delete all reports when keep is 0 (default)", async () => {
    ...
    expect(console.info).toHaveBeenCalledWith(expect.stringContaining("Deleted 3 report files"));
  ...
  it("should keep specified number of recent reports", async () => {
    ...
    expect(console.info).toHaveBeenCalledWith(expect.stringContaining("Deleted 1 report file"));
    const remainingFiles = Object.keys(vol.toJSON()).filter((path) => path.includes(".jsonl"));
  ...
  it("should not delete reports when keep count equals total", async () => {
    ...
    expect(console.info).toHaveBeenCalledWith(expect.stringContaining("Deleted 0 report files"));
    const remainingFiles = Object.keys(vol.toJSON()).filter((path) => path.includes(".jsonl"));
