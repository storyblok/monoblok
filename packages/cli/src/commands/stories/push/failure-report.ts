...
const ENRICHABLE_FIELDS = [
  ...
  "id",
  "filename",
] as const satisfies readonly (keyof FailedStoryRecord)[];
...
function enrichFieldMessage(detail: string, record: FailedStoryRecord): string {
  ...
  const field = match[1];
  if (!ENRICHABLE_FIELDS.includes(field as (typeof ENRICHABLE_FIELDS)[number])) {
    return detail;
  }
...
function renderFailureReport(ui: UI, records: FailedStoryRecord[], verbose: boolean): void {
  ...
  for (const record of records) {
    ...
    const messages =
      record.error instanceof APIError ? record.error.messageStack : [record.error.message];
