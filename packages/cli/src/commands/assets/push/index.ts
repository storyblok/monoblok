...
import {
  collectAssetInternalTagNames,
  ensureAssetInternalTags,
...
import {
  ...
  upsertAssetFoldersPipeline,
  upsertAssetsPipeline,
} from "../pipelines";
...
const pushCmd = assetsCommand
  ...
  .addOption(
    new Option(
      "--target <target>",
      "push destination: space | shared | auto (default: space for a single asset, auto for a bulk push)",
    ).choices(PUSH_TARGETS),
  )
  .option(
    "--library <libraryId>",
    "destination library ID (required for single-asset --target=shared)",
  )
  .option(
    "--cleanup",
    "delete local assets and metadata after a successful push (note: does not cleanup manifests)",
  )
...
pushCmd.action(async (assetInput, options, command) => {
  const ui = getUI();
  ...
  if (assetBinaryPath) {
    if (target === "auto") {
      ...
      handleError(
        new CommandError(
          "Pushing a single asset requires --target=space or --target=shared, not --target=auto.",
        ),
        verbose,
      );
      return;
  ...
  if (assetBinaryPath) {
    ...
    const shortFilename =
      options.shortFilename || assetDataPartial.short_filename || sourceBasename;
  ...
  try {
    for (const scope of scopes) {
      ...
      if (!assetBinaryPath) {
        ...
        const cleanupAssetFolderTransport =
          options.cleanup && !options.dryRun
            ? makeCleanupAssetFolderFSTransport()
            : () => Promise.resolve();

        summaries.push(
          ...(await upsertAssetFoldersPipeline({
      ...
      const resolveSharedTagIds =
        scope.kind === "library" && !options.dryRun
      ...
      // omits it) would 403.
      const baseCreateAsset =
        scope.kind === "library" && !options.dryRun
          ? (() => {
              const { libraryId } = scope;
              return async (asset: AssetUpload, fileBuffer: ArrayBuffer) => {
                const withFolder =
                  asset.asset_folder_id == null ? { ...asset, asset_folder_id: libraryId } : asset;
      ...
      const createAssetTransport = options.dryRun
        ...
        : baseCreateAsset;
      const updateAssetTransport = options.dryRun ? async () => {} : transports.updateAsset;
      const assetManifestTransport = options.dryRun
        ? () => Promise.resolve()
        : makeAppendAssetManifestFSTransport({ manifestFile });
      const cleanupAssetTransport =
        options.cleanup && !options.dryRun
          ? makeCleanupAssetFSTransport()
          : () => Promise.resolve();
      ...
      let assetInternalTagsByName =
        scope.kind === "space" && fromSpace !== targetSpace
     ...
     */
    if (spaceAssetMap) {
      const hasUpdatedFilename = (entry: {
        old: Asset | AssetMapped | AssetUpload;
        new: AssetMapped;
      ...
      const hasUpdatedAssets = spaceAssetMap
        .values()
        .some((v) => hasUpdatedFilename(v) || hasMetadata(v));
      if (hasUpdatedAssets && options.updateStories) {
        const schemas = await findComponentSchemas(
        ...
        summaries.push(
          ...(await mapAssetReferencesInStoriesPipeline({
            logger,
