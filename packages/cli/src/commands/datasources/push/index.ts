...
const pushCmd = datasourcesCommand
  ...
  .option(
    "--su, --suffix <suffix>",
    "Load only files matching *.<suffix>.json (e.g. datasources.dev.json)",
  )
      ...
      for (const datasource of spaceState.local.datasources) {
        ...
        if (result) {
          ...
          for (let i = 0; i < localEntries.length; i++) {
            ...
            try {
              const upserted = await upsertDatasourceEntry(
                space,
                result.id,
                entry,
                existingEntryId,
              ...
              if (entryId != null && entry.dimension_values !== undefined) {
                ...
                const dimensionCodes = new Set([
                  ...Object.keys(localDimensionValues),
                  ...Object.keys(targetDimensionValues),
                ]);
                ...
                for (const code of dimensionCodes) {
                  ...
                  if (!dimension?.id) {
                    dimensionWarnings.push(
                      `Skipping dimension "${code}" for entry "${entry.name}": no matching dimension in target space ${space}`,
                    );
                    continue;
                  }
                  await updateDatasourceEntryDimension(
                    space,
                    entryId,
                    entry,
                    dimension.id,
