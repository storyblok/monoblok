...
pullCmd.action(async (options, command) => {
  ...
  try {
    const fetchStoryPagesProgress = ui.createProgressBar({
      title: "Fetching Story Pages...".padEnd(23),
    });
      ...
      writeStoryStream({
        transports: {
          writeStory: options.dryRun
            ...
            : makeWriteStoryFSTransport({
                directoryPath: resolveCommandPath(directories.stories, space, basePath),
              }),
        },
        onIncrement: () => {
    ...
    ui.info(
      `Pull results: ${summary.save.total} stories pulled, ${Math.max(summary.fetchStories.failed, summary.save.failed)} stories failed`,
    );
    ui.list([
