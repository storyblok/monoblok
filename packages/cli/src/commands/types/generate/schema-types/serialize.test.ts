...
describe("serializeBlockDefinition", () => {
  ...
  it("maps component_whitelist to an allow tuple", () => {
    ...
    expect(result.definitionBody).toContain(
      "{ name: 'body'; type: 'bloks'; allow: ['grid', 'teaser'] }",
    );
  });
  ...
  it("emits is_nestable true when the wire omits it, and false when explicit", () => {
    expect(
      serializeBlockDefinition(component({ is_nestable: undefined }), emptyContext).definitionBody,
  ...
  it("omits allow when the restriction is switched off, for names and for groups", () => {
    const names = serializeBlockDefinition(
      component({
        schema: {
    ...
    const groups = serializeBlockDefinition(
      component({
        schema: {
          body: {
            type: "bloks",
  ...
  it("emits allow on richtext but not on other whitelisted field types", () => {
    const richtext = serializeBlockDefinition(
      component({
    ...
    const multilink = serializeBlockDefinition(
      component({
