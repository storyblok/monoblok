...
import type {
  Block as PluginBlock,
  Story as PluginStory,
} from "./__fixtures__/expected-types-with-plugins";
...
describe("emitted Story/StoryMapi thread FieldPlugins", () => {
  it("resolves a registered custom field through Block<TName>", () => {
    expectTypeOf<NonNullable<PluginBlock<"page">["accent"]>>()
      .toHaveProperty("hex")
      .toEqualTypeOf<string>();
  });

  it("resolves a registered custom field through Story, not just through Block<TName>", () => {
    expectTypeOf<NonNullable<PluginStory["content"]["accent"]>>()
      .toHaveProperty("hex")
      .toEqualTypeOf<string>();
  });
});
