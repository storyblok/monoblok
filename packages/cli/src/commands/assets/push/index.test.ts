...
const server = setupServer(
  http.get("https://mapi.storyblok.com/v1/spaces/:spaceId/assets/:assetId", () =>
    HttpResponse.json({ message: "Not Found" }, { status: 404 }),
  ),
  http.get("https://mapi.storyblok.com/v1/spaces/:spaceId/asset_folders/:folderId", () =>
    HttpResponse.json({ message: "Not Found" }, { status: 404 }),
  ),
  http.get("https://mapi.storyblok.com/v1/spaces/:spaceId/internal_tags", () =>
  ...
  http.get("https://mapi.storyblok.com/v1/spaces/:spaceId/shared_asset_folders", () =>
...
const preconditions = {
  canLoadComponents(components: MockComponent[], space = DEFAULT_SPACE, basePath?: string) {
    ...
    vol.fromJSON(
      Object.fromEntries(
        components.map((c) => [join(componentsDir, `${c.name}.json`), JSON.stringify(c)]),
      ),
    );
  },
  canLoadAssets(
    assets: MockAsset[],
    { space = DEFAULT_SPACE, basePath }: { space?: string; basePath?: string } = {},
  ) {
  ...
  canLoadAssetsManifest(
    manifestEntries: Record<number | string, number | string>[],
    { space = DEFAULT_SPACE, basePath }: { space?: string; basePath?: string } = {},
  ) {
    const manifestPath = join(
      resolveCommandPath(directories.assets, space, basePath),
      "manifest.jsonl",
    );
  ...
  canCreateRemoteFolders(
    ...
    server.use(
      http.post(
        `https://mapi.storyblok.com/v1/spaces/${space}/asset_folders`,
        async ({ request }) => {
  ...
  failsToCreateRemoteFolders({ space = DEFAULT_SPACE }: { space?: string } = {}) {
    server.use(
      http.post(`https://mapi.storyblok.com/v1/spaces/${space}/asset_folders`, () =>
        HttpResponse.json({ message: "Creation failed" }, { status: 500 }),
      ),
    );
  },
  canFetchRemoteFolders(
    folders: MockAssetFolder[],
    { space = DEFAULT_SPACE }: { space?: string } = {},
  ) {
  ...
  canUpdateRemoteFolders(
    ...
    server.use(
      http.put(
        `https://mapi.storyblok.com/v1/spaces/${space}/asset_folders/:folderId`,
        async ({ params }) => {
          const match = remoteFolders.find(
            (folder) => String(folder.id) === String(params.folderId),
          );
          if (!match) {
  ...
  failsToUpdateRemoteFolders({ space = DEFAULT_SPACE }: { space?: string } = {}) {
    server.use(
      http.put(`https://mapi.storyblok.com/v1/spaces/${space}/asset_folders/:folderId`, () =>
        HttpResponse.json({ message: "Update failed" }, { status: 500 }),
      ),
    );
  },
  hasAssetInternalTags(
  ...
  canCreateAssetInternalTags({
    space = DEFAULT_SPACE,
    idByName = {},
  }: { space?: string; idByName?: Record<string, number> } = {}) {
    const spy = vi.fn();
    server.use(
      http.post(
        `https://mapi.storyblok.com/v1/spaces/${space}/internal_tags`,
        async ({ request }) => {
          const body = (await request.json()) as {
            internal_tag: { name: string; object_type?: string };
          };
          spy(body);
  ...
  canUpsertRemoteAssets(
    ...
    server.use(
      ...
      http.get(
        `https://mapi.storyblok.com/v1/spaces/${space}/assets/:assetId/finish_upload`,
        ({ params }) => {
          ...
          server.use(
            ...
            http.get(
              `https://mapi.storyblok.com/v1/spaces/${space}/assets/:assetId`,
              ({ params }) => {
                const match = remoteAssets.find(
                  (asset) => String(asset.id) === String(params.assetId),
                );
                if (!match) {
  ...
  failsToCreateRemoteAssets({ space = DEFAULT_SPACE }: { space?: string } = {}) {
    server.use(
      http.post(`https://mapi.storyblok.com/v1/spaces/${space}/assets`, () =>
        HttpResponse.json({ message: "Creation failed" }, { status: 500 }),
      ),
    );
  },
  failsToUpdateRemoteAssets({ space = DEFAULT_SPACE }: { space?: string } = {}) {
    server.use(
      http.patch(`https://mapi.storyblok.com/v1/spaces/${space}/assets/:assetId`, () =>
        HttpResponse.json({ message: "Update failed" }, { status: 500 }),
      ),
    );
  },
  failsToUploadAssets() {
    server.use(
      http.post("https://s3.amazonaws.com/a.storyblok.com", () =>
  ...
  canUpsertSharedAssets(
    assets: MockAsset[],
    { space = DEFAULT_SPACE, libraryId = 7 }: { space?: string; libraryId?: number } = {},
  ) {
    ...
    server.use(
      ...
      // Step 3: finish upload.
      http.get(
        `https://mapi.storyblok.com/v1/spaces/${space}/shared_assets/:assetId/finish_upload`,
        () => HttpResponse.json({ message: "Upload finalized" }),
      ),
      // Step 4: retrieve + metadata update target.
      http.get(
        `https://mapi.storyblok.com/v1/spaces/${space}/shared_assets/:assetId`,
        ({ params }) => {
          const match = remoteAssets.find((asset) => String(asset.id) === String(params.assetId));
          if (!match) {
            return HttpResponse.json({ message: "Asset not found" }, { status: 404 });
      ...
      http.put(`https://mapi.storyblok.com/v1/spaces/${space}/shared_assets/:assetId`, () =>
        HttpResponse.json({}),
      ),
    );
    return remoteAssets;
...
describe("assets push command", () => {
  ...
  it("should push assets and asset folders and map to new asset folder IDs", async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining("Push results: 1 processed, 0 assets failed"),
    );
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining("Folders: 1/1 succeeded, 0 failed."),
    );
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining("Assets: 1/1 succeeded, 0 failed."),
    );
  ...
  it("should carry the dimensions segment from the source filename into the create payload as `size`", async () => {
    const targetSpace = "54321";
    const asset = makeMockAsset({
      filename: `https://a.storyblok.com/f/${DEFAULT_SPACE}/2048x1820/7fb286a4c5/photo.jpg`,
    });
    preconditions.canLoadAssets([asset]);
    preconditions.canUpsertRemoteAssets([asset], { space: targetSpace });
  ...
  it("should not set `size` when the source filename has no dimensions segment", async () => {
    const targetSpace = "54321";
    const asset = makeMockAsset({
      filename: `https://a.storyblok.com/f/${DEFAULT_SPACE}/7fb286a4c5/photo.jpg`,
    });
    preconditions.canLoadAssets([asset]);
    preconditions.canUpsertRemoteAssets([asset], { space: targetSpace });
  ...
  it("should correctly resolve parent IDs even when child folders precede parents", async () => {
    ...
    for (const folder of folders) {
      const expectedRemoteParentId = folder.parent_id
        ? localToRemoteId.get(folder.parent_id)
        : undefined;
  ...
  it("should update asset folders that already exist instead of creating them again", async () => {
    ...
    expect(actions.updateAssetFolder).toHaveBeenCalledWith(
      remoteFolder.id,
      expect.objectContaining({
  ...
  it("should log an error and stop when manifest loading fails", async () => {
    const manifestPath = join(
      resolveCommandPath(directories.assets, DEFAULT_SPACE),
      "manifest.jsonl",
    );
  ...
  it("should use sidecar json data when pushing a single local asset without --data", async () => {
    ...
    preconditions.canLoadLocalFile(
      assetJsonPath,
      JSON.stringify({
        meta_data: {
          alt: "Alt text",
  ...
  it("should download an external asset and clean up the temp file after upload", async () => {
    ...
    const tempFiles = Object.keys(vol.toJSON()).filter((filename) =>
      filename.startsWith(`${tempRoot}/`),
    );
  ...
  it("should delete local assets and asset folders when cleanup is enabled", async () => {
    ...
    const files = Object.keys(vol.toJSON()).filter((filename) =>
      filename.startsWith(`${assetsDir}${sep}`),
    );
  ...
  it("should derive short_filename from filename CDN URL when short_filename is not present", async () => {
    ...
    const userAssetMetadata = {
      filename: "https://a.storyblok.com/f/12345/500x500/hero.png",
      alt: "Hero",
    };
    vol.fromJSON({
      [join(assetsDir, "hero.json")]: JSON.stringify(userAssetMetadata),
  ...
  it("should filter stories using search parameter when updating a single asset", async () => {
    ...
    expect(storyActions.fetchStories).toHaveBeenCalledWith(
      DEFAULT_SPACE,
      expect.objectContaining({
  ...
  it("should translate source-space internal_tag_ids to target-space tag IDs by sidecar tag name", async () => {
    ...
    preconditions.canLoadFolders([]);
    preconditions.canLoadAssets([asset]);
    preconditions.hasAssetInternalTags([{ id: sourceTagId, name: "stale-source-name" }], {
      space: DEFAULT_SPACE,
    });
    preconditions.hasAssetInternalTags([{ id: targetTagId, name: "shared-tag" }], {
      space: targetSpace,
    });
    preconditions.canUpsertRemoteAssets([asset], { space: targetSpace });
  ...
  it("should create internal tags missing from the target space and map to the new tag ID", async () => {
    ...
    expect(createTagSpy).toHaveBeenCalledWith(
      expect.objectContaining({
        internal_tag: expect.objectContaining({ name: "missing-tag", object_type: "asset" }),
      }),
    );
    expect(actions.createAsset).toHaveBeenCalledWith(
  ...
  it("should drop internal tag IDs without sidecar tag metadata with an ID warning", async () => {
    ...
    preconditions.canLoadFolders([]);
    preconditions.canLoadAssets([asset]);
    preconditions.hasAssetInternalTags([{ id: 999, name: "shared-tag" }], { space: targetSpace });
    preconditions.canUpsertRemoteAssets([asset], { space: targetSpace });
    ...
    expect(console.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Dropped 1 unknown internal asset tag not present in target space: #333",
      ),
    );
    expect(process.exitCode).not.toBe(1);
  });
  ...
  it("should drop the tag reference and not fail the push when creating a missing tag fails", async () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Dropped 1 unknown internal asset tag not present in target space: uncreatable-tag",
      ),
    );
    expect(process.exitCode).not.toBe(1);
  });
  ...
  describe("shared library targets", () => {
    it("pushes a single asset to a library (--target shared --library)", async () => {
      ...
      preconditions.canUpsertSharedAssets(
        [makeMockAsset({ short_filename: "hero.png", filename: "hero.png" })],
        { libraryId: 7 },
      );
    ...
    it("defaults a bulk shared-asset folder to the library root when the sidecar omits it", async () => {
      preconditions.hasLibraries([{ id: 7, name: "Brand", accessLevel: "write" }]);
      preconditions.canUpsertSharedAssets(
        [makeMockAsset({ short_filename: "x.png", filename: "x.png" })],
        { libraryId: 7 },
      );
      const dir = resolveCommandPath(directories.assets, join("shared", "7"));
      vol.fromJSON({
        [join(dir, "x_2.png")]: "binary",
        [join(dir, "x_2.json")]: JSON.stringify({
          id: 2,
          short_filename: "x.png",
          filename: "x.png",
        }),
      });
    ...
    it("skips the library root folder on bulk push, still pushes child folders", async () => {
      ...
      server.use(
        ...
        http.get("https://mapi.storyblok.com/v1/spaces/12345/shared_asset_folders/7", () => {
          ...
          return HttpResponse.json({
            shared_asset_folder: { id: 7, name: "Brand", parent_id: null },
          });
        }),
        http.put("https://mapi.storyblok.com/v1/spaces/12345/shared_asset_folders/7", () => {
          rootFolderTouched = true;
          return HttpResponse.json(
            { message: "Cannot update shared root asset folder in space context" },
            { status: 403 },
          );
        }),
        ...
        http.post(
          "https://mapi.storyblok.com/v1/spaces/12345/shared_asset_folders",
          async ({ request }) => {
            childFolderCreated = true;
            const body = (await request.json()) as {
              shared_asset_folder: { name: string; parent_id?: number };
            };
            return HttpResponse.json({
              shared_asset_folder: {
                id: 80,
                name: body.shared_asset_folder.name,
                parent_id: body.shared_asset_folder.parent_id,
              },
            });
          },
        ),
      ...
      const dir = resolveCommandPath(directories.assets, join("shared", "7"));
      vol.fromJSON({
        [join(dir, "folders", "Brand_7.json")]: JSON.stringify({
          id: 7,
          name: "Brand",
          parent_id: null,
          uuid: "u7",
        }),
        [join(dir, "folders", "Sub_8.json")]: JSON.stringify({
          id: 8,
          name: "Sub",
          parent_id: 7,
          uuid: "u8",
    ...
    it("updates an existing child library folder with only whitelisted fields", async () => {
      ...
      server.use(
        ...
        http.put(
          "https://mapi.storyblok.com/v1/spaces/12345/shared_asset_folders/8",
          async ({ request }) => {
            updatePayload = (
              (await request.json()) as { shared_asset_folder: Record<string, unknown> }
            ).shared_asset_folder;
      ...
      vol.fromJSON({
        [join(dir, "folders", "Brand_7.json")]: JSON.stringify({
          ...
          parent_id: null,
          uuid: "u7",
          asset_folder_access: [{ space_id: 12345, access_level: "write" }],
          regions: ["eu"],
        }),
        [join(dir, "folders", "Sub_8.json")]: JSON.stringify({
          id: 8,
    ...
    it("round-trips meta_data for a shared asset", async () => {
      ...
      server.use(
        http.put(
          "https://mapi.storyblok.com/v1/spaces/12345/shared_assets/:assetId",
          async ({ request }) => {
    ...
    it("creates missing library tags and remaps internal_tag_ids on push", async () => {
      ...
      server.use(
        http.get("https://mapi.storyblok.com/v1/spaces/12345/shared_internal_tags", () =>
          HttpResponse.json({ internal_tags: [] }),
        ),
        http.post("https://mapi.storyblok.com/v1/spaces/12345/shared_internal_tags", () =>
          HttpResponse.json({ internal_tag: { id: 500, name: "hero", object_type: "asset" } }),
        ),
        http.put(
          "https://mapi.storyblok.com/v1/spaces/12345/shared_assets/:assetId",
          async ({ request }) => {
      ...
      vol.fromJSON({
        ...
        [join(dir, "x_2.json")]: JSON.stringify({
          ...
          short_filename: "x.png",
          filename: "x.png",
          internal_tag_ids: ["300"],
          internal_tags_list: [{ id: 300, name: "hero" }],
        }),
      });
    ...
    it("paginates shared library tags so a match on a later page is reused, not recreated", async () => {
      ...
      server.use(
        ...
        http.get(
          "https://mapi.storyblok.com/v1/spaces/12345/shared_internal_tags",
          ({ request }) => {
        ...
        http.post("https://mapi.storyblok.com/v1/spaces/12345/shared_internal_tags", () => {
          createCalled = true;
          return HttpResponse.json({
            internal_tag: { id: 500, name: "hero", object_type: "asset" },
          });
        }),
        http.put(
          "https://mapi.storyblok.com/v1/spaces/12345/shared_assets/:assetId",
    ...
    it("should treat a missing shared-assets directory as empty (no warn, no log error)", async () => {
      ...
      const stderrCalls = (console.error as ReturnType<typeof vi.fn>).mock.calls.map(
        (args) => args[0],
      );
      expect(stderrCalls.some((msg: unknown) => typeof msg === "string" && msg.includes("⚠"))).toBe(
        false,
      );
      expect(
