...
export function createComponentFoldersResource<DefaultThrowOnError extends boolean = false>(
  ...
  return {
    list<ThrowOnError extends boolean = DefaultThrowOnError>(
      options: {
        query?: ListComponentGroupsData["query"];
        signal?: AbortSignal;
        throwOnError?: ThrowOnError;
        fetchOptions?: FetchOptions;
      } & SpaceIdPathOverride = {},
    ): Promise<ApiResponse<ListComponentGroupsResponses[200], ThrowOnError>> {
      const { query, signal, path, throwOnError, fetchOptions } = options;
      const resolvedSpaceId = getSpaceId(path);
      return wrapRequest<ListComponentGroupsResponses[200], ThrowOnError>(
        () =>
          mapi.listComponentGroups({
            client,
    ...
    ): Promise<ApiResponse<GetComponentGroupResponses[200], ThrowOnError>> {
    ...
    create<ThrowOnError extends boolean = DefaultThrowOnError>(
      options: {
        body: CreateComponentGroupData["body"];
        signal?: AbortSignal;
        throwOnError?: ThrowOnError;
        fetchOptions?: FetchOptions;
      } & SpaceIdPathOverride,
    ): Promise<ApiResponse<CreateComponentGroupResponses[201], ThrowOnError>> {
      const { body, signal, path, throwOnError, fetchOptions } = options;
      const resolvedSpaceId = getSpaceId(path);
      return wrapRequest<CreateComponentGroupResponses[201], ThrowOnError>(
        () =>
          mapi.createComponentGroup({
            client,
            path: { space_id: resolvedSpaceId },
            body,
            signal,
    ...
    update<ThrowOnError extends boolean = DefaultThrowOnError>(
      componentGroupId: number,
      options: {
        body: UpdateComponentGroupData["body"];
        signal?: AbortSignal;
        throwOnError?: ThrowOnError;
        fetchOptions?: FetchOptions;
      } & SpaceIdPathOverride,
