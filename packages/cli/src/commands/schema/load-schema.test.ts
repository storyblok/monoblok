  ...
  it("should classify a mixed module and map blocks to the wire schema shape", () => {
    const moduleExports = {
      pageBlock: {
        ...
        created_at: "",
        updated_at: "",
        fields: [{ name: "title", type: "text", pos: 0 }],
      },
      heroBlock: {
        name: "hero",
        id: 1,
...
describe("classifyExports duplicate identities", () => {
  ...
  it("should not throw when the same definition is exported directly and via the schema object", () => {
    ...
    const result = classifyExports({
      hero,
      colors,
      schema: { blocks: { hero }, datasources: { colors } },
    });
...
describe("classifyExports folders", () => {
  ...
  it("should throw on duplicate slugified leaf names under different parents", () => {
    expect(() =>
      ...
      /Duplicate folder name "heros" \(folders "layout\/heros" and "marketing\/heros"\): Storyblok group names must be unique per space/,
    );
  });
  ...
  it("should unwrap a schema object that only has folders", () => {
    ...
    expect(data.folders).toEqual([{ name: "Layout", path: "layout", parentPath: null }]);
  });
});
