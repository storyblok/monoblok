  ...
  describe("loginWithEmailAndPassword", () => {
    it("should get if the user requires otp", async () => {
      ...
      const result = await loginWithEmailAndPassword(
        "julio.professional@storyblok.com",
        "password",
        "eu",
      );
    ...
    it("should throw an error for invalid credentials", async () => {
      await expect(
        loginWithEmailAndPassword("david.bisbal@storyblok.com", "password", "eu"),
    ...
    it("should login successfully with valid email, password, and otp", async () => {
      server.use(
        http.post("https://mapi.storyblok.com/v1/users/login", async ({ request }) => {
          ...
          if (
            body?.email === "julio.professional@storyblok.com" &&
            body?.password === "password" &&
            body?.otp_attempt === "123456"
          ) {
