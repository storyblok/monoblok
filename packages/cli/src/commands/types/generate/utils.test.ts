...
describe("detectUsedDatasourceTypes", () => {
  ...
  it("should detect multiple datasource types", () => {
    const content =
      "export interface Page { country: CountriesDataSource; category: CategoriesDataSource; }";
    const datasourceResults = [
      { title: "CountriesDataSource", content: "", isComponent: false, isDatasource: true },
      { title: "CategoriesDataSource", content: "", isComponent: false, isDatasource: true },
    ];
...
describe("generateComponentImports", () => {
  it("should generate all necessary imports for a component", () => {
    ...
    expect(
      imports.some((i) => i.includes("import type { ISbStoryData } from '@storyblok/js';")),
    ).toBe(true);

    // Should have Storyblok types import
    expect(
      imports.some((i) => i.includes("StoryblokAsset") && i.includes("from '../storyblok.d.ts'")),
    ).toBe(true);

    // Should have datasource import
    expect(
      imports.some(
        (i) => i.includes("CountriesDataSource") && i.includes("from './datasource-types.d.ts'"),
      ),
    ).toBe(true);
  ...
  it("should generate imports for multiple referenced components", () => {
    ...
    expect(imports.some((i) => i.includes("import type { Hero } from './Hero.d.ts'"))).toBe(true);
    expect(imports.some((i) => i.includes("import type { Footer } from './Footer.d.ts'"))).toBe(
      true,
    );
    expect(imports.some((i) => i.includes("import type { Sidebar } from './Sidebar.d.ts'"))).toBe(
      true,
    );
  });
});
...
describe("createDatasourcesFile", () => {
  it("should create datasources file with content", () => {
    const datasourceResults = [
      {
        title: "CountriesDataSource",
        content: 'export type CountriesDataSource = "usa" | "uk";',
        isComponent: false,
        isDatasource: true,
      },
      {
        title: "CategoriesDataSource",
        content: 'export type CategoriesDataSource = "tech" | "news";',
        isComponent: false,
        isDatasource: true,
      },
    ];
