...
const preconditions = {
  canCreateAssetWithSize({ space = "123" }: { space?: string } = {}) {
    ...
    server.use(
      http.post(`https://mapi.storyblok.com/v1/spaces/${space}/assets`, ({ request }) => {
      ...
      http.get(`https://mapi.storyblok.com/v1/spaces/${space}/assets/:asset_id/finish_upload`, () =>
        HttpResponse.json({}),
      ),
      http.get(`https://mapi.storyblok.com/v1/spaces/${space}/assets/:asset_id`, () =>
        HttpResponse.json({
          id: 1,
...
describe("assets.convertToShared()", () => {
  it("should post to the convert endpoint with target_asset_folder_id and return the converted asset", async () => {
    ...
    server.use(
      http.post(
        "https://mapi.storyblok.com/v1/spaces/:space_id/assets/:asset_id/convert",
        ({ request }) => {
  ...
  it("should successfully retrieve a single asset", async () => {
    server.use(
      http.get("https://mapi.storyblok.com/v1/spaces/:space_id/assets/:asset_id", () => {
        return HttpResponse.json({
          asset: { id: 456, filename: "https://a.storyblok.com/f/1/x/hero.png" },
        });
      }),
    );
    const client = createManagementApiClient({
...
describe("assets.upload()", () => {
  it("should perform the full sign -> S3 upload -> finalize -> get flow", async () => {
    ...
    server.use(
      ...
      http.get(
        "https://mapi.storyblok.com/v1/spaces/:space_id/assets/:signed_response_object_id/finish_upload",
        () => {
