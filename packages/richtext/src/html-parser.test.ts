...
describe("hTML → Richtext: Nodes", () => {
  describe("emoji", () => {
    it("parses an emoji span", () => {
      const html =
        '<span data-type="emoji" data-name="rocket" data-emoji="🚀"><img src="https://cdn.example.com/rocket.png" alt="🚀"></span>';
  ...
  describe("table", () => {
    ...
    it("parses table cells with colspan and rowspan", () => {
      const html =
        '<table><tbody><tr><td colspan="2">Span 2 cols</td></tr><tr><td rowspan="2">Span 2 rows</td><td>Normal</td></tr></tbody></table>';
      const result = htmlToStoryblokRichtext(html);
    ...
    it("parses table cells with marks inside", () => {
      const html =
        "<table><tbody><tr><td><strong>Bold cell</strong></td><td><em>Italic cell</em></td></tr></tbody></table>";
      const result = htmlToStoryblokRichtext(html);
...
describe("hTML → Richtext: Marks", () => {
  describe("link", () => {
    ...
    it("parses a story link with data-uuid and data-linktype", () => {
      const result = htmlToStoryblokRichtext(
        '<p><a href="/about" data-uuid="abc-123" data-linktype="story">About</a></p>',
      );
      expect(result.content[0].content[0].marks[0].attrs).toMatchObject({
        href: "/about",
        uuid: "abc-123",
    ...
    it("parses a story link with data-anchor", () => {
      const result = htmlToStoryblokRichtext(
        '<p><a href="/page" data-uuid="uuid-1" data-anchor="section" data-linktype="story">Link</a></p>',
      );
      expect(result.content[0].content[0].marks[0].attrs).toMatchObject({
        href: "/page",
        uuid: "uuid-1",
    ...
    it("parses an email link", () => {
      const result = htmlToStoryblokRichtext(
        '<p><a href="mailto:info@example.com" data-linktype="email">Email us</a></p>',
      );
      expect(result.content[0].content[0].marks[0].attrs).toMatchObject({
    ...
    it("parses an asset link", () => {
      const result = htmlToStoryblokRichtext(
        '<p><a href="https://a.storyblok.com/f/000/doc.pdf" data-linktype="asset">Download</a></p>',
      );
      expect(result.content[0].content[0].marks[0].attrs).toMatchObject({
...
describe("hTML → Richtext: Options & configuration", () => {
  describe("styled mark with styleOptions", () => {
    ...
    it("ignores unrecognized style classes", () => {
      const result = htmlToStoryblokRichtext('<p><span class="unknown-style">text</span></p>', {
        ...
        content: [
          {
            ...
            content: [
              ...
              {
                type: "text",
                marks: [{ type: "styled", attrs: { class: "style-1" } }],
                text: "bar",
              },
            ],
          },
          {
            ...
            content: [
              ...
              {
                type: "text",
                marks: [{ type: "styled", attrs: { class: "style-2" } }],
                text: "qux",
              },
            ],
          },
  ...
  describe("data loss warnings", () => {
    it("warns the user when transformation leads to data loss", () => {
      ...
      const unsupportedAttributes =
        '<p id="foo" class="unsupported">Hello <a target="_blank" href="/home">world!</a></p>';
      const unsupportedStyles = '<p>Hello <span class="supported unsupported">world!</span></p>';
      htmlToStoryblokRichtext([unsupportedAttributes, unsupportedStyles].join(""), {
        styleOptions: [{ name: "supported", value: "supported" }],
      });

      expect(warn).toHaveBeenCalledWith(
        expect.stringContaining(
          '[StoryblokRichText] - `id` "foo" on `<p>` can not be transformed to rich text.',
        ),
      );
      expect(warn).toHaveBeenCalledWith(
        expect.stringContaining(
          '[StoryblokRichText] - `class` "unsupported" on `<p>` can not be transformed to rich text.',
        ),
      );
      expect(warn).toHaveBeenCalledWith(
        expect.stringContaining(
          '[StoryblokRichText] - `class` "unsupported" on `<span>` can not be transformed to rich text.',
        ),
      );
    });
  });
...
describe("roundtrip: HTML → Richtext → HTML", () => {
  ...
  it("roundtrips a completely new node type", () => {
    ...
    const output = richTextResolver({ tiptapExtensions: extensions }).render(json);
    expect(output).toBe(
      '<div data-callout="" data-type="warning" class="callout-warning">Watch out!</div>',
    );
  });
  ...
  it("roundtrips a custom mark with extra attributes", () => {
    ...
    const output = richTextResolver({ tiptapExtensions: extensions }).render(json);
    expect(output).toBe(
      '<p><mark data-color="pink" style="background-color: pink">Important</mark></p>',
    );
  });
});
...
describe("hTML → Richtext: Editor compatibility (reported issues)", () => {
  describe("links must be marks on text nodes, never standalone nodes", () => {
    it("renders links as text nodes with link marks, not as link nodes", () => {
      const result = htmlToStoryblokRichtext(
        '<p>Click <a href="https://example.com">here</a> please.</p>',
      );
  ...
  describe("telephone links", () => {
    it("parses tel: links correctly", () => {
      const result = htmlToStoryblokRichtext(
        '<p><a href="tel:+44 3457 911 911">+44 3457 911 911</a></p>',
      );
      const linkText = result.content[0].content[0];
    ...
    it("parses tel: links with target and title attributes", () => {
      const result = htmlToStoryblokRichtext(
        '<p><a href="tel:+44 3457 911 911" target="" title="">+44 3457 911 911</a></p>',
      );
      const linkText = result.content[0].content[0];
