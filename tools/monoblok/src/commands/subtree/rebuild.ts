      ...
      try {
        ...
        if (entries.length > 1) {
          if (!options?.yes) {
            ...
            console.log(
              chalk.yellow(`Multiple packages match, automatically selected: ${selectedEntry[0]}`),
            );
          }
        } else {
          selectedEntry = entries[0];
          ...
          const { confirm } = await inquirer.prompt([
            {
              ...
              name: "confirm",
              message: chalk.red(
                `Are you sure you want to rebuild ${name}? This will DELETE and re-add ${remoteDir}`,
              ),
              default: false,
            },
          ]);
