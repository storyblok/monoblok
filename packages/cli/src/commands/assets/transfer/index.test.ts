...
const preconditions = {
  canTransferAssets({ space = DEFAULT_SPACE }: { space?: string } = {}) {
    server.use(
      http.post(
        `https://mapi.storyblok.com/v1/spaces/${space}/assets/:assetId/convert`,
        ({ params }) => {
  ...
  canListAssets(
    ...
    { space = DEFAULT_SPACE, onRequest }: { space?: string; onRequest?: (url: URL) => void } = {},
  ) {
    server.use(
...
describe("assets transfer command", () => {
  ...
  it("should report a count summary rather than one row per asset", async () => {
    ...
    expect(actions.transferAssets).toHaveBeenCalledWith(
      DEFAULT_SPACE,
      [42, 43],
      7,
      expect.anything(),
    ...
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining("2 transferred, 0 failed (of 2)"),
    );
  ...
  it("should group repeated failure reasons into a single counted line", async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining("0 transferred, 3 failed (of 3)"),
    );
  ...
  it("should forward --query filters to the asset list request and transfer the filtered set", async () => {
    ...
    preconditions.canListAssets([{ id: 42 }], {
      onRequest: (url) => {
        seen = url;
      },
    });
