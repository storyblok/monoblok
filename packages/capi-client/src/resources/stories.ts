...
export function createStoriesResource<InlineRelations extends boolean>(deps: StoriesResourceDeps) {
  ...
  return {
    get: async <ThrowOnError extends boolean = false>(
      ...
      const resolvedQuery =
        typeof identifier === "string" && UUID_RE.test(identifier) && !query.find_by
      ...
      return requestWithCache<GetResponse<InlineRelations>, ThrowOnError>(
        "GET",
        requestPath,
        resolvedQuery,
        async (requestQuery: Record<string, unknown>): Promise<Res> => {
          const response = await throttleManager.execute(requestPath, requestQuery, () =>
            asApiResponse<GetResponse<InlineRelations>, ThrowOnError>(
              get({
                client,
                path: { identifier },
                query: requestQuery,
    ...
    list: async <ThrowOnError extends boolean = false>(
      ...
      return requestWithCache<ListResponse<InlineRelations>, ThrowOnError>(
        "GET",
        requestPath,
        query,
        async (requestQuery: Record<string, unknown>): Promise<ResAll> => {
          const response = await throttleManager.execute(requestPath, requestQuery, () =>
            asApiResponse<ListResponse<InlineRelations>, ThrowOnError>(
              list({
                client,
                query: requestQuery,
                signal,
