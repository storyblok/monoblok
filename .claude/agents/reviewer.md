...
Read your `MEMORY.md` at the start of each review. Update it when you discover patterns worth
remembering.
...
**Quality**: small/focused functions, descriptive names, no dead code, no magic numbers, helpful
error messages.

**Architecture**: fits existing patterns, no unnecessary coupling, proper separation (actions vs
commands vs utils), appropriate types/interfaces.

**Reusability**: duplicated logic extracted, existing helpers used, constants centralized.

**Best practices**: consistent async/await, errors propagated (not swallowed), edge cases handled,
no security issues.

**API integration** (when applicable): error codes handled (401, 403, 404, 422, 429),
request/response formats match backend, rate limiting considered.
