...
> [!WARNING] The `--filename` option is ignored when using `--separate-files`. Each datasource will
> be saved with its own name.
...
- Entries include a `dimension_values` map with the value for every defined dimension, keyed by the
  dimension code (`entry_value`, e.g. `en`). The command fetches one page set per dimension, so
  datasources with many dimensions issue more requests.
