...
describe("validateStory", () => {
  ...
  describe("custom field plugins", () => {
    ...
    it("errors instead of silently passing when a plugin ships an async validator", () => {
      ...
      const asyncPlugin = defineFieldPlugin({
        fieldType: "storyblok-colorpicker",
        value: asyncValue,
      });
      const result = validateStory(
...
const validateMultilink = (cta: unknown) =>
  validateStory(
    {
...
describe("validateStory — constraints", () => {
  ...
  it("errors when a number is below min_value or above max_value", () => {
    expect(validate({ rating: "0" }).issues.some((i) => i.code === "constraint_violation")).toBe(
      true,
    );
    expect(validate({ rating: "6" }).issues.some((i) => i.code === "constraint_violation")).toBe(
      true,
    );
  ...
  it("errors when a number has more decimal places than decimals allows", () => {
    ...
    expect(
      validateStory({ content: { component: "priced", price: "9.999" } }, s).issues.some(
        (i) => i.code === "constraint_violation",
      ),
    ).toBe(true);
  ...
  it("errors when a number is not a multiple of steps", () => {
    ...
    expect(
      validateStory({ content: { component: "stepped", amount: "2.3" } }, s).issues.some(
        (i) => i.code === "constraint_violation",
      ),
    ).toBe(true);
  ...
  it("errors when text exceeds max_length or is below minlength", () => {
    expect(
      validate({ title: "toolong" }).issues.some((i) => i.code === "constraint_violation"),
    ).toBe(true);
    expect(validate({ title: "a" }).issues.some((i) => i.code === "constraint_violation")).toBe(
      true,
    );
  });
  ...
  it("errors when options count is outside min_options/max_options", () => {
    ...
    expect(
      validate({ tags: ["a", "b", "c"] }).issues.some((i) => i.code === "constraint_violation"),
    ).toBe(true);
  });

  it("errors when multiasset count is outside minimum_entries/maximum_entries", () => {
    expect(validate({ gallery: [] }).issues.some((i) => i.code === "constraint_violation")).toBe(
      true,
    );
    expect(
      validate({ gallery: [asset(), asset(), asset()] }).issues.some(
        (i) => i.code === "constraint_violation",
      ),
    ).toBe(true);
  });
  ...
  it("errors when bloks count is outside minimum/maximum", () => {
    expect(validate({ items: [] }).issues.some((i) => i.code === "constraint_violation")).toBe(
      true,
    );
    expect(
      validate({
      ...
      }).issues.some((i) => i.code === "constraint_violation"),
    ).toBe(true);
  });
...
describe("validateStory — richtext allow entries", () => {
    ...
    const result = validateStory(
      {
        content: {
          component: "page",
          body: richtextWith([{ _uid: "uid-1", component: "hero", title: "Hi" }]),
        },
      },
      schema,
    );
  ...
  it("rejects an embedded blok outside the allowed folder", () => {
    const result = validateStory(
      {
        content: {
          component: "page",
          body: richtextWith([{ _uid: "uid-1", component: "teaser", text: "hi" }]),
        },
      },
      schema,
    );
    const disallowed = result.issues.find((i) => i.code === "disallowed_component");
    expect(disallowed).toBeDefined();
    expect(disallowed?.path).toEqual([
      "content",
      "body",
      "content",
      0,
...
describe("validateStory — declared options", () => {
  ...
  it("rejects an option value that is not declared", () => {
    ...
    expect(issue?.message).toBe(
      'Value "bronze" is not one of the options declared for field "tier": "gold", "silver".',
    );
  });
  ...
  it("skips fields whose options are resolved in the space", () => {
    ...
    const result = validateStory(
      { content: { component: "themed", theme: "anything" } },
      { blocks: { themed } },
    );
    expect(result.ok).toBe(true);
  });
  ...
  it("skips a field that declares no options", () => {
    ...
    expect(
      validateStory({ content: { component: "free", tier: "whatever" } }, { blocks: { free } }).ok,
    ).toBe(true);
  });
});
      ...
      tbody: [
        {
          ...
          body: [{ _uid: "c", component: "_table_col", value: "Cell" }],
        },
      ],
    });
    expect(result.issues).toEqual([]);
...
describe("validateStory — the empty string counts as unset", () => {
  ...
  it.each(["num", "when", "flag", "cover", "link", "body"])(
    'accepts an optional %s left as ""',
    (name) => {
  ...
  it('still reports a required field left as ""', () => {
    ...
    const result = validateStory(
      { content: { component: "required", cover: "" } },
      { blocks: { required } },
    );
    ...
    const result = validateStory(
      {
        content: {
          ...
          body: {
            type: "doc",
            content: [{ type: "blok", attrs: { body: [{ _uid: "u", component: "ghost" }] } }],
          },
        },
      },
      s,
    ...
    const result = validateStory(
      {
        content: {
          ...
          body: {
            type: "doc",
            content: [
              { type: "blok", attrs: { body: [{ _uid: "u", component: "teaser", text: "hi" }] } },
            ],
          },
        },
      },
