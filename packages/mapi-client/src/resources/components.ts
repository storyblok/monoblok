...
export function createComponentsResource<DefaultThrowOnError extends boolean = false>(
  ...
  return {
    ...
    ): Promise<ApiResponse<ListComponentsResult, ThrowOnError>> {
      const { query, signal, path, throwOnError, fetchOptions } = options;
      const resolvedSpaceId = getSpaceId(path);
      return wrapRequest<ListComponentsResult, ThrowOnError>(
        () =>
          mapi.listManagementComponents({
            client,
    ...
    ): Promise<ApiResponse<GetComponentResult, ThrowOnError>> {
    ...
    ): Promise<ApiResponse<CreateComponentResult, ThrowOnError>> {
      const { body, signal, path, throwOnError, fetchOptions } = options;
      const resolvedSpaceId = getSpaceId(path);
      return wrapRequest<CreateComponentResult, ThrowOnError>(
        () =>
          mapi.createComponent({
            client,
            path: { space_id: resolvedSpaceId },
            body,
            signal,
    ...
    ): Promise<ApiResponse<DeleteComponentResponses[200], ThrowOnError>> {
      const { signal, path, throwOnError, fetchOptions } = options;
      const resolvedSpaceId = getSpaceId(path);
      return wrapRequest<DeleteComponentResponses[200], ThrowOnError>(
        () =>
          mapi.deleteComponent({
            client,
            path: { space_id: resolvedSpaceId, id: String(componentId) },
            signal,
            ...(throwOnError === undefined ? {} : { throwOnError }),
    ...
    restore<ThrowOnError extends boolean = DefaultThrowOnError>(
      componentId: number,
      options: {
        signal?: AbortSignal;
        throwOnError?: ThrowOnError;
    ...
    ): Promise<ApiResponse<RestoreComponentResult, ThrowOnError>> {
      const { signal, path, throwOnError, fetchOptions } = options;
      const resolvedSpaceId = getSpaceId(path);
      return wrapRequest<RestoreComponentResult, ThrowOnError>(
        () =>
          mapi.restoreComponent({
            client,
    ...
    restoreVersion<ThrowOnError extends boolean = DefaultThrowOnError>(
      ...
      options: {
        query?: Omit<RestoreVersionData["query"], "model" | "model_id">;
        signal?: AbortSignal;
        throwOnError?: ThrowOnError;
        fetchOptions?: FetchOptions;
      } & SpaceIdPathOverride = {},
