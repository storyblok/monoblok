...
export function buildDependencyGraph(context: GraphBuildingContext): DependencyGraph {
  ...
  spaceState.local.presets.forEach((preset) => {
    ...
    if (!sourceComponent) {
      ...
      getLogger().warn(
        `Preset "${preset.name}" (ID: ${preset.id}) references component ID ${preset.component_id} which is not available in local data. Skipping preset.`,
      );
      return;
    }
...
export function validateGraph(graph: DependencyGraph): void {
  ...
  if (circularWhitelists.length > 0) {
    logger.info(
      `Circular component whitelists detected (allowed): ${circularWhitelists.join(", ")}`,
    );
  }
