...
function spaceData(
  ...
  extras: Partial<SpaceComponentsData> = {},
): SpaceComponentsData {
...
describe("collectAllDependencies (option A)", () => {
  it("drops sibling components referenced via component_whitelist", () => {
    ...
    const { filteredComponents } = collectAllDependencies([checkout], [checkout, richtext], [], []);

    expect(filteredComponents.map((c) => c.name)).toEqual(["checkout"]);
  });

  it("keeps the assigned group and its ancestors", () => {
    const child: ComponentFolder = {
      id: 20,
      uuid: "child-uuid",
      name: "Child",
...
describe("resolveGroupSelector", () => {
  it("resolves an unambiguous name to its subtree of uuids", () => {
    expect([...resolveGroupSelector(groups, "Checkout")].sort()).toEqual([
      "checkout",
      "checkout-forms",
    ]);
  });
  ...
  it("includes descendants of the resolved group", () => {
    expect([...resolveGroupSelector(groups, "Marketing")].sort()).toEqual([
      "checkout",
      "checkout-forms",
      "marketing",
    ]);
...
describe("resolveTagSelector", () => {
  ...
  it("maps names to ids", () => {
    expect([...resolveTagSelector(tags, ["beta", "checkout-team"])].sort((a, b) => a - b)).toEqual([
      10, 11,
    ]);
  });
