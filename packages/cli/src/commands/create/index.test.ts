...
describe("createCommand", () => {
  describe("--token option", () => {
    it("should use provided token, skip space creation, and update env", async () => {
      ...
      expect(handleEnvFileCreation).toHaveBeenCalledWith(
        expect.any(String),
        "my-access-token",
        "eu",
        "react",
  ...
  describe("template validation", () => {
    ...
    it("should accept valid template via deprecated --blueprint flag with warning", async () => {
      ...
      expect(mockedUI.warn).toHaveBeenCalledWith(
        "The --blueprint flag is deprecated. Please use --template instead.",
      );
      expect(generateProject).toHaveBeenCalledWith("react", "my-project", expect.any(String));
    });
  ...
  describe("interactive mode", () => {
    ...
    it("should validate project path input correctly", async () => {
      ...
      expect(mockValidate("   ")).toBe("Project path is required");
      expect(mockValidate("invalid/project/name!")).toBe(
  ...
  describe("project generation", () => {
    ...
    it("should handle browser opening failure gracefully", async () => {
      ...
      vi.mocked(generateSpaceUrl).mockReturnValue(
        "https://app.storyblok.com/#/me/spaces/12345/dashboard",
      );
  ...
  describe("skip space functionality", () => {
    ...
    it("should still show appropriate messages when skipping space creation", async () => {
      ...
      expect(mockedUI.ok).not.toHaveBeenCalledWith(
        "Created .env file with Storyblok access token",
        true,
      );
      ...
      it("should show generic success message when creating in personal account", async () => {
        ...
        expect(mockedUI.ok).toHaveBeenCalledWith(
          expect.stringMatching(
            /^Storyblok space created, preview url and \.env configured automatically/,
          ),
        );
        expect(mockedUI.ok).not.toHaveBeenCalledWith(expect.stringContaining("organization"));
        expect(mockedUI.ok).not.toHaveBeenCalledWith(expect.stringContaining("partner portal"));
      });
    });
    ...
    describe("user data fetching", () => {
      it("should handle user data fetching failure", async () => {
        ...
        expect(mockedUI.error).toHaveBeenCalledWith(
          "Failed to fetch user info. Your session may have expired.",
        );
        expect(generateProject).toHaveBeenCalled();
        expect(createSpace).not.toHaveBeenCalled();
      });
      ...
      expect(handleError).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining(
            'Cannot create space in region "us". Your account is configured for region "eu"',
          ),
        }),
      );
