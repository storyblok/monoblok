...
function isUpToDateWithRemote(branch) {
  try {
    ...
    const remoteCommit = execCommand(`git rev-parse origin/${branch}`, {
      silent: true,
      ignoreError: true,
    });
...
async function main() {
  ...
  if (!isUpToDateWithRemote(currentBranch)) {
    log(
      `❌ Error: Your ${currentBranch} branch is not up to date with origin/${currentBranch}`,
      RED,
    );
    log("\n📋 Instructions:", BLUE);
  ...
  if (projectsArg) {
    ...
    if (droppedByBranch.length > 0) {
      log(
        `\n⚠️  Removing requested projects not eligible on ${currentBranch}: ${droppedByBranch.join(", ")}`,
        YELLOW,
      );
    }
    if (kept.length === 0) {
      log(
        `\n❌ Error: None of the requested projects are configured to release from ${currentBranch}`,
        RED,
      );
      log("\n📋 Instructions:", BLUE);
      log(`  1. Check each package's ${YELLOW}release.branches${RESET} in its package.json`);
      log(
        `  2. Switch to a branch the project(s) support, or add ${YELLOW}${currentBranch}${RESET} to their ${YELLOW}release.branches${RESET}\n`,
      );
      exit(1);
    }
  ...
  try {
    ...
    if (versionArg) {
      releaseCommand += ` ${versionArg}`;
    }
    releaseCommand += " --skip-publish";
    if (isPrerelease) {
      releaseCommand += ` --preid=${currentBranch}`;
    }
    releaseCommand += ` --projects=${effectiveProjects}`;
    if (firstRelease) {
      releaseCommand += " --first-release";
    }
    if (isDryRun) {
    ...
    if (isDryRun) {
      ...
      log(
        `  4. Click "Run workflow" to publish to npm${isPrerelease ? ` with the ${YELLOW}${currentBranch}${RESET} tag` : ""}\n`,
      );
    }
  } catch {
    log("\n━".repeat(50), BLUE);
...
function snapshotPackageVersions(names, packagesDir = "packages") {
  ...
  for (const entry of readdirSync(packagesDir)) {
    ...
    if (!pkg.name || !wanted.has(pkg.name)) {
      continue;
    }
