...
describe("formatSchemaIssues", () => {
  it("should render both sections with alphabetically sorted entries and story lists", () => {
    ...
    expect(message).toContain(
      "These fields will be lost when the stories are pushed. To fix, either:",
    );
    expect(message).toContain(
    ...
    expect(message.indexOf("Missing component schemas")).toBeLessThan(
      message.indexOf("Fields not declared"),
    );
...
describe("hasSchemaIssues", () => {
  it("should return false when both drift and missing sets are empty", () => {
    expect(
      hasSchemaIssues({ driftByComponent: new Map(), missingSchemas: new Map(), total: 3 }),
    ).toBe(false);
  });
