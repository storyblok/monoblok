...
export const makeMockAsset = (
  ...
  return {
    ...
    filename:
      overrides.filename ||
      `https://s3.amazonaws.com/a.storyblok.com/f/${space}/500x500/${shortFilename}`,
...
export const assetFileExists = (asset: MockAsset) => {
  ...
  const binaryFilename = getAssetBinaryFilename(
    asset as Parameters<typeof getAssetBinaryFilename>[0],
  );
  const metadataFilename = getAssetFilename(asset as Parameters<typeof getAssetFilename>[0]);
  const binary = Object.entries(vol.toJSON()).find(([filename]) =>
    filename.endsWith(binaryFilename),
  )?.[1];
  const metadata = Object.entries(vol.toJSON()).find(([filename]) =>
    filename.endsWith(metadataFilename),
  )?.[1];
...
export const folderFileExists = (folder: MockAssetFolder) => {
  return Object.keys(vol.toJSON()).some((filename) =>
    filename.endsWith(`folders/${getFolderFilename(folder)}`),
  );
};
