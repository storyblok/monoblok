...
describe("extractAssetSizeFromFilename", () => {
  it("extracts the dimensions segment from a CDN URL", () => {
    expect(
      extractAssetSizeFromFilename(
        "https://a.storyblok.com/f/329189/2048x1820/7fb286a4c5/image.jpg",
      ),
    ).toBe("2048x1820");
  });
  ...
  it("returns undefined when the URL has no dimensions segment", () => {
    expect(
      extractAssetSizeFromFilename(
        "https://a.storyblok.com/f/293255674717942/8ae82d3a12/image.jpg",
      ),
    ).toBeUndefined();
  });
  ...
  it("extracts the dimensions segment when it uses an uppercase X", () => {
    expect(
      extractAssetSizeFromFilename("https://a.storyblok.com/f/329189/860X645/7aa24cb39f/image.jpg"),
    ).toBe("860X645");
  });
...
describe("internalTagNamesFromAssets", () => {
  it("collects unique tag names in first-seen order, ignoring blanks", () => {
    const assets: Parameters<typeof internalTagNamesFromAssets>[0] = [
      {
        internal_tags_list: [
          { id: 1, name: "blue" },
          { id: 2, name: "green" },
      ...
      },
      {
        internal_tags_list: [
          { id: 3, name: "blue" },
          { id: 4, name: "" },
          { id: 5 } as unknown as { id: number; name: string },
        ],
      },
      {},
    ];
    const names = internalTagNamesFromAssets(assets);
