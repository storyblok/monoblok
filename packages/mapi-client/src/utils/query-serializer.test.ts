...
describe("querySerializer", () => {
  it("should serialize primitives like the default", () => {
    expect(querySerializer({ per_page: 100, page: 1, starts_with: "en/" })).toBe(
      "per_page=100&page=1&starts_with=en%2F",
    );
  });
  ...
  it("should serialize a nested filter_query as bracket notation", () => {
    expect(decode(querySerializer({ filter_query: { highlighted: { in: "true" } } }))).toBe(
