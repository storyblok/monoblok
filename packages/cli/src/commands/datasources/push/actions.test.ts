  ...
  describe("readDatasourcesFiles", () => {
    ...
    describe("separate files (content-detected)", () => {
      ...
      it("should read datasources from separate files with suffix", async () => {
        ...
        vol.fromJSON({
          ...
          "/mock/path/datasources/source-space/categories.dev.json":
            JSON.stringify(mockDatasource2),
    ...
    describe("consolidated files (content-detected)", () => {
      ...
      it("should read consolidated file with custom filename", async () => {
        ...
        vol.fromJSON({
          "/mock/path/datasources/source-space/my-data.json": JSON.stringify([
            mockDatasource1,
            mockDatasource2,
          ]),
        });
      ...
      server.use(
        http.post("https://mapi.storyblok.com/v1/spaces/12345/datasources", async ({ request }) => {
          ...
          return HttpResponse.json(
            { datasource: { id: 99, name: "Countries", slug: "countries" } },
            { status: 201 },
          );
        }),
      );
    ...
    it("should omit dimensions_attributes for a datasource without dimensions", async () => {
      ...
      server.use(
        http.post("https://mapi.storyblok.com/v1/spaces/12345/datasources", async ({ request }) => {
          createdBody = await request.json();
          return HttpResponse.json(
            { datasource: { id: 1, name: "colors", slug: "colors" } },
            { status: 201 },
          );
        }),
      );
