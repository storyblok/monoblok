...
export const heroBaseSchema = [{ ...headlineField, required: true }, imageField, ...ctaFields()];
...
export const heroCompactBlock = defineBlock({
  ...
  is_nestable: true,
  fields: [defineField("headline", { type: "text", max_length: 60 }), imageField, ...ctaFields()],
});
...
export const heroNoCta = defineBlock({
  ...
  is_nestable: true,
  fields: [{ ...headlineField, required: true }, imageField],
});
