...
import {
  buildLibraryRootResolver,
  listReadableLibraries,
  resolveScopeBaseDir,
  type Scope,
} from "../scope";
...
const pullCmd = assetsCommand
  ...
  .addOption(
    new Option("--target <target>", "pull source: with-referenced | all | space | shared")
      .choices(PULL_TARGETS)
      .default("with-referenced"),
  )
  .option("-d, --dry-run", "Preview changes without applying them to Storyblok")
  .option(
    "-q, --query <query>",
    'Filter assets using Storyblok filter query syntax. Example: --query="search=my-file.jpg&with_tags=tag1,tag2"',
  )
  .option("--asset-token <token>", "Asset token for accessing private assets")
  .description(
    `Download your space's assets, and optionally shared library assets, as local files.`,
  );
...
pullCmd.action(async (options, command) => {
  ...
  const pullScopeFull = async (scope: Scope): Promise<Set<number>> => {
    ...
    const fetchAssetPagesProgress = ui.createProgressBar({
      title: "Fetching Asset Pages...".padEnd(24),
    });
    ...
    await pipeline(
      ...
      writeAssetFolderStream({
        ...
        onFolderError: (error, folder) => {
          summary.folderResults.failed += 1;
          summary.folderResults.total = Math.max(
            summary.folderResults.total,
            summary.folderResults.succeeded + summary.folderResults.failed,
          );
  ...
  try {
    if (target === "space") {
      ...
      const stories = await readLocalStoryContents(
