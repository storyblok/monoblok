...
deleteCmd.action(async (name: string, options: DeleteDatasourceOptions, command: Command) => {
  konsola.title(
  ...
  try {
    ...
    if (options.id) {
      ...
      konsola.ok(
        `Datasource ${chalk.hex(colorPalette.DATASOURCES)(options.id)} deleted successfully from space ${space}.`,
      );
    } else {
      // Delete by name
      ...
      spinner.succeed(`Datasource deleted`);
      konsola.ok(
        `Datasource ${chalk.hex(colorPalette.DATASOURCES)(name)} deleted successfully from space ${space}.`,
      );
    }
  } catch (error) {
